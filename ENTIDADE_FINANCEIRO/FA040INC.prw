...
/*
FA040INC
Ponto de entrada na inclusao do titulo a receber validando a obrigatoriedade do centro de custo /item / classe de valor de acordo com a conta contabil
...
IF !lF040Auto
    ...
    If (lCc .And. Empty(M->E1_CCUSTO)) 
        lRet := .F.
        Help('', 1, 'FA040INC01',, 'Centro de custo obrigatorio para essa conta contabil.', 1, 0)
    EndIf

    If    (lItem .And. Empty(M->E1_ITEMCTA)) 
        lRet := .F.
        Help('', 1, 'FA040INC02',, 'Item obrigatorio para essa conta contabil.', 1, 0)
    EndIf

    If (lClvl .And. Empty(M->E1_CLVL))
        lRet := .F.
        Help('', 1, 'FA040INC03',, 'Classe de valor obrigatorio para essa conta contabil. ', 1, 0)
    EndIf     
Endif
