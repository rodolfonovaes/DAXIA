...
PRIVATE cFILEWK,cIMPMEMO,cEXPMEMO,;
        aNEW      := {},;
        aVEL      := {},;
        lINGLES   := UPPER(EasyGParam("MV_AVG0037",,"INGLES")) $ UPPER(WORKID->EEA_IDIOMA),;
...
Begin Sequence   
 ...
 //HEADER_P->AVG_C02_20:= AllTrim(EEC->EEC_PREEMB) // DFS - 30/05/11 - Inclusão do número do Embarque.
   ...
   If Type("lDocImpInv") <> "U" .And. lDocImpInv  // LRS - 08/03/2016
   ...
   WkMsgP52->(DbGoTop())   
   Do While WkMsgP52->(!Eof())
      If ! Empty(WkMsgP52->WKMARCA)
         cWkTexto  := WkMsgP52->WKTEXTO
         ///cWkTexto  := RTRIM(STRTRAN(WkMsgP52->WKTEXTO,ENTER," "))
         ...
         If nLinhasSp > 0
            ...
            If nLinhasSp > 1
               For nX := 2 To nLinhasSp
                   ...
                   DETAIL_P->AVG_C03_60 := MemoLine(WkMsgP52->WKTEXTO,TAMSPEC,nX)
               Next                 
            EndIf
         EndIF
      EndIf
...
Local bShow    := {|nTela,o| if(nTela==2,dbSelectArea("WkMsgP52"),;
...
Begin Sequence
   ...
   If cAcao == "NEW"
      ...
      aOld    := {Select(), E_CriaTrab("EE4",aSemSX3,"WkMsgP52")}
      
      // ** Carrega as Specifications que foram impressas na última impressão.
      IF ! EMPTY(cWKTEXTO)
         WkMsgP52->(DBAPPEND())
         WkMsgP52->WKMARCA    := cMARCA
         WkMsgP52->EE4_TIPMEN := "REIMPRESSAO"
         FOR I := 1 TO MLCOUNT(cWKTEXTO,TAMSPEC)
             WkMsgP52->WKTEXTO := WkMsgP52->WKTEXTO+MEMOLINE(cWKTEXTO,TAMSPEC,I)+ENTER
         NEXT
      ENDIF
            ...
            FOR I := 1 TO MLCOUNT(cTexto,AVSX3("EE4_VM_TEX",AV_TAMANHO))
                ...
                IF ! EMPTY(Z)
                   FOR Y := 1 TO MLCOUNT(Z,TAMSPEC)
                       WkMsgP52->WKTEXTO := WkMsgP52->WKTEXTO+RTRIM(MEMOLINE(z,TAMSPEC,Y))+" "
                   NEXT
                ELSE
                   WkMsgP52->WKTEXTO := WkMsgP52->WKTEXTO+ENTER+ENTER
                ENDIF
            NEXT
            WkMsgP52->EE4_TIPMEN := EE4->EE4_TIPMEN
            WkMsgP52->EE4_COD    := EE4->EE4_COD
         EndIf
         EE4->(DbSkip())
     EndDo
     dbSelectArea("WkMsgP52")
     WkMsgP52->(dbGoTop())
     aCampos     := {{"WKMARCA",," "},;
                     ColBrw("EE4_COD","WkMsgP52"),;
                     ColBrw("EE4_TIPMEN","WkMsgP52"),;
                     {{|| MemoLine(WkMsgP52->WKTEXTO,AVSX3("EE4_VM_TEX",AV_TAMANHO),1)},"",AVSX3("EE4_VM_TEX",AV_TITULO)}}
     
     //GFP 25/10/2010
     aCampos := AddCpoUser(aCampos,"EE4","2")
     
     oMark       := MsSelect():New("WkMsgP52","WKMARCA",,aCampos,lInverte,@cMarca,{18,3,175,312}) 
     oMark:bAval := {|| EditObs(cMarca), oMark:oBrowse:Refresh() }      
     xRet        := oMark                                                
     RestOrd(aOrd)
   Elseif cAcao == "END"
      If Select("WkMsgP52") > 0
         WkMsgP52->(E_EraseArq(aOld[2]))
      Endif
...
IF WkMsgP52->(!Eof())
   IF Empty(WkMsgP52->WKMARCA)
      ...
      IF nOpc == 1
         IF !Empty(nMarcado)
            nRec := WkMsgP52->(RecNo())
            WkMsgP52->(dbGoTo(nMarcado))
            WkMsgP52->WKMARCA := Space(2)
            WkMsgP52->(dbGoTo(nRec))
         Endif
         cObs := cObs + CMemo
         WkMsgP52->WKTEXTO := cMemo
         WkMsgP52->WKMARCA := cMarca
         nMarcado := nRec
      Endif
   Else
...
Begin Sequence
   ...
   IndRegua("WKTOT",cFILEWK+TEOrdBagExt(),"WKINDICE","AllwaysTrue()","AllwaysTrue()","Processando")
   c1 := CriaTrab(Nil,.f.)

   IndRegua("WKTOT",c1+TEOrdBagExt(),"WKDESPESA","AllwaysTrue()","AllwaysTrue()","Processando")
   SET INDEX TO (cFILEWK+TEOrdBagExt()),(c1+TEOrdBagExt())
...
Begin Sequence
   ...
   DEFINE MSDIALOG oDlg TITLE ALLTRIM(WORKID->EEA_TITULO) FROM 0,0 TO 320,380 OF oMainWnd PIXEL
     
      oPanel:= TPanel():New(0, 0, "", oDlg,, .F., .F.,,, 90, 90) //MCF - 11/01/2016
      oPanel:Align:= CONTROL_ALIGN_ALLCLIENT

	      oFLD := TFolder():New(13,5.5,{STR0041,STR0042},{"IMP","EXP"},oPanel,,,,.T.,.F.,180,108) //"&Importador"###"&Exportador"
      IF nP_LINHA > 0
