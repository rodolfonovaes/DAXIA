...
Begin Sequence
   ...
   DEFINE MSDIALOG oDlg TITLE STR0002 FROM 9,0 TO /*20*/30,51 OF oMainWnd //"Packing List"  // GFP - 18/05/2012 - Ajuste de tamanho de tela para versao 11.5
     
     oPanel:= TPanel():New(0, 0, "",oDlg,, .F., .F.,,, 40.5, 7.5)   
     
     If lMarDesc
        oFld := TFolder():New(15,1,{STR0003,STR0004,"Descrição da Mercadoria"},{"PAR","ASS","DES"},oPanel,,,,.T.,.F.,200,90) //"Parametros","Assinatura"
     Else
        oFld := TFolder():New(15,1,{STR0003,STR0004},{"PAR","ASS"},oPanel,,,,.T.,.F.,200,90) //"Parametros","Assinatura"\
     EndIf
