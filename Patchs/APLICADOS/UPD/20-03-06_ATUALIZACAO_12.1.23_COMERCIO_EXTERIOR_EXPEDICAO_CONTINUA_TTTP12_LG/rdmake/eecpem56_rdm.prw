...
Begin Sequence
n1:=n2:=n3:=n4:=n5:=n6:=0
   DEFINE MSDIALOG oDlg TITLE AllTrim(WorkId->EEA_TITULO) FROM 200,1 TO 720,600 PIXEL OF oMainWnd
      
      oPanel:= TPanel():New(0, 0, "",oDlg,, .F., .F.,,, 300, 197)

      oFld := TFolder():New(13,1,{STR0021,STR0022,STR0023,STR0024,STR0025},; //"&Dados Gerais"###"&Anexos"###"&Mensagem"###"&Instruções Especiais"###"&Cambio"
                                 {"DAD","ANE","MSG","INS","PAR"},oPanel,,,,.T.,.F.,300,197)//300,160
      aFld := oFld:aDialogs
      ...
      // ** Folder Anexos.
      oFLDF:=aFLD[2]
      @ 006,043 GET yy OF oFldF
      aHeader := a_aHeader
      aCols   := a_aCols
      n       := a_n
      oGetDb  := IW_MultiLine(65,3,249,300,.T.,.T.)
   
      // ** Folder Mensagens.
      oMark2 := Observacoes(STR0038,cMarca,cWKTEXTO) //"New"
      @ 006,043 GET xx OF oFld:aDialogs[3]
      aHeader := m_aHeader
      aCols   := m_aCols
      n       := m_n
...
Begin Sequence
   ...
   IF cAcao == "NEW"
      ...
      oMark       := MsSelect():New("WkMsg","WKMARCA",,aCampos,lInverte,@cMarca,{65,3,249,300},,,)
...
Begin Sequence
   ...
   DEFINE MSDIALOG oDlg TITLE ALLTRIM(WORKID->EEA_TITULO) FROM 0,0 TO 240,357 OF oMainWnd PIXEL
      ...
      @ 5,5 GET cEXPMEMO MEMO SIZE 170,80 OF oPanel PIXEL HSCROLL
      oPanel:Align:= CONTROL_ALIGN_ALLCLIENT
