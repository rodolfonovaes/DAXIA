...
Local lMarDesc  := EasyGParam("MV_AVG0200",,.F.) .And. EYH->(FieldPos("EYH_CODMAR")) > 0  
...
Begin Sequence
   ...
   DEFINE MSDIALOG oDlg TITLE STR0002 FROM 9,0 TO /*20*/25.5,51 OF oMainWnd //"Packing List"  // GFP - 18/05/2012 - Ajuste de tamanho de tela para versao 11.5
   
     If lMarDesc
        oFld := TFolder():New(30,1,{STR0003,STR0004,"Descrição da Mercadoria"},{"PAR","ASS","DES"},oDlg,,,,.T.,.F.,200,90) //"Parametros","Assinatura"
     Else
        oFld := TFolder():New(30,1,{STR0003,STR0004},{"PAR","ASS"},oDlg,,,,.T.,.F.,200,90) //"Parametros","Assinatura"\
     EndIf
...
Begin Sequence
   ...
   While EX9->(!EOF()) .and. EX9->(EX9_FILIAL+EX9_PREEMB) == xFilial("EX9")+EEC->EEC_PREEMB
      ...
      IF !(EX9->(FieldPos("EX9_GRANEL")) > 0 .And. EX9->EX9_GRANEL == '1')
   ...
   While EX9->(!EOF()) .and. EX9->(EX9_FILIAL+EX9_PREEMB) == xFilial("EX9")+EEC->EEC_PREEMB
      ...
      //Código do Container
      HEADER_P->AVG_C01_20 := If( EX9->(FieldPos("EX9_GRANEL")) > 0 .And. EX9->EX9_GRANEL == '1', "" , Alltrim(EX9->EX9_CONTNR) )  //NCF - 29/08/2013 - Não exibe dados do Conteiner caso seja granel
      ...
      For n := 1 to Len(aPallet)
         ...
         While EYH->(!Eof() .And. EYH_FILIAL+EYH_ESTUF+EYH_PREEMB+EYH_IDVINC == xFilial("EYH")+"S"+EEC->EEC_PREEMB+cPalletID)
            ...
            If EE5->(DbSeek(xFilial("EE5")+EYH->EYH_CODEMB))
               EE2->(dbSetOrder(1))
               EE2->(dbSeek(xFilial()+MC_TEMB+TM_GER+AvKey(EEC->EEC_IDIOMA,"EE2_IDIOMA")+AvKey(EYH->EYH_CODEMB,"EE2_COD")))
			   
               //Descrição
               If !lItem // GFP - 17/05/2012 - Se não for produto, colunas "Packages" são preenchidas.
                  DETAIL_P->AVG_C01_60 := AllTrim(MSMM(EE2->EE2_TEXTO,AvSX3("EE2_VM_TEX",AV_TAMANHO)))//AllTrim(EYH->EYH_DESEMB)
               Else
                  DETAIL_P->AVG_C01_60 := ""
               EndIf   
            ...
            //NCF - 29/08/2013 - Peso Líq. Unitario
            DETAIL_P->AVG_C01_20 := Transform(EYH->EYH_PSLQUN,"@E 999,999,999.999")
            
            //NCF - 29/08/2013 - Peso Bru. Unitario            
            DETAIL_P->AVG_C07_20 := Transform(EYH->EYH_PSBRUN,"@E 999,999,999.999")
            ...
            For nInc := 1 To Len(aItens)
               ...
               nPesLiqUn := aItens[nInc][8]
               nPesBru   := aItens[nInc][10]
               nPesBruUn := aItens[nInc][9]
               DETAIL_P->AVG_C08_20 := Transform(nPesLiq,AVSX3("EE9_PSLQTO",AV_PICTURE))
               DETAIL_P->AVG_C01_20 := Transform(nPesLiqUn,AVSX3("EE9_PSLQTO",AV_PICTURE))
               DETAIL_P->AVG_C09_20 := Transform(nPesBru,AVSX3("EE9_PSLQTO",AV_PICTURE))
               DETAIL_P->AVG_C07_20 := Transform(nPesBruUn,AVSX3("EE9_PSLQTO",AV_PICTURE))
               nPesoLqTo += nPesLiq
      ...
      If !Empty(   If( EEC->(FieldPos("EEC_LC_NUM")) > 0,EEC->EEC_LC_NUM,"" )   ) 
         ...
         If nLinObsCDC > 0     
            For nX:=1 To nLinObsCDC
               ...
               //Identificador da Linha(Utilizado para o Sub-Relatório)
               DETAIL_P->AVG_N01_04 := nIncCont
               DETAIL_P->AVG_C01150 := MemoLine(cObsCrtCre,AVSX3("EEL_VM_TEX",3),nX)  
            Next     
         EndIf
         RestOrd(aOrdEEL)
         nIncCont++
...
Begin Sequence
   ...
   */
   ...
   If !lItem  // GFP - 18/05/2012
         ...
         While EYH->(!Eof() .And. EYH_FILIAL+EYH_ESTUF+EYH_PREEMB+EYH_IDVINC == cChave)
            If !Empty(EYH->EYH_COD_I)
               ...
               If EE9->(DbSeek(xFilial("EE9")+EYH->(EYH_PREEMB+EYH_SEQEMB)))
                  ...
                  If lDescMarc .AND. !Empty(MSMM(EYH->EYH_CODMAR, TAMSX3("EYH_MARCAC")[1],,, LERMEMO))
                     nQtd := 1
                  Else
                     nQtd := EYH->EYH_QTDEMB
                  EndIf
         
                  aAdd(aRet,{cDesc,EYH->EYH_QTDEMB,EE9->EE9_COD_I,EE9->EE9_UNIDAD,nPsLqUn,EE9->EE9_REFCLI, EYH->EYH_LOTE,EYH->EYH_PSLQUN,EYH->EYH_PSBRUN,EYH->EYH_PSBRTO})
               
               //NCF - 30/07/2013 - O produto pode não existir na EE9 por pertencer à estrutura de um produto constante
               //                   entre os itens da EE9 que foi quebrado por estrutura na Estufagem.  
               ElseIf SB1->(DbSeek(xFilial("SB1")+EYH->EYH_COD_I))
                  cDesc   := BuscaDesc("SB1") 
                  nPsLqUn := AvTransUnid(SB1->B1_UM,cUnidad,SB1->B1_COD,SB1->B1_PESO) * EYH->EYH_QTDEMB         
                  aAdd(aRet,{cDesc,EYH->EYH_QTDEMB,SB1->B1_COD,SB1->B1_UM,nPsLqUn,"", EYH->EYH_LOTE,EYH->EYH_PSLQUN,EYH->EYH_PSBRUN,EYH->EYH_PSBRTO})
               EndIf
            Else
               aRetI := BuscaItens()
               For i := 1 To Len(aRetI)
                  aAdd(aRet,aClone(aRetI[i]))
               Next i
            EndIf
      ...
      cMItensEmb := IF(  !EMPTY(EYH->EYH_CODMAR) , MSMM(EYH->EYH_CODMAR, TAMSX3("EYH_MARCAC")[1],,, LERMEMO) , "" )
      If EE5->(DBSeek(xFilial("EE5")+EYH->EYH_CODEMB)) .And. !EMPTY(cMItensEmb) .And. !EYH->(DbSeek(cChave))     //NCF - 02/07/2013 - Descricao dos itens contidos para embalagens que não possuam itens 
         EYH->(DbGoto(nRecno))
         aAdd(aRet,{MSMM(EYH->EYH_CODMAR, TAMSX3("EYH_MARCAC")[1],,, LERMEMO),0,"","",0,"", ""})                 //                   ou embalagens contidas nela, pois neste caso a descrição é a marcação da embalagem.
      EndIf                                                                                          
...
Static Function BuscaDesc(cOrigem)
Local cDesc := ""
Local cDescItem := ""
Local cDescMarc := ""
Local cCapaEmb := ""
...
Else 
   ...
   If !Empty(cCodMemIte)
      cDescItem := MSMM(cCodMemIte,AVSX3("EE2_VM_TEX",3),,,LERMEMO)
   Else
...
EndIf
   ...
   If lDescMarc .AND. !Empty(cDescMarc)
      cDesc := cDescMarc
   EndIf
