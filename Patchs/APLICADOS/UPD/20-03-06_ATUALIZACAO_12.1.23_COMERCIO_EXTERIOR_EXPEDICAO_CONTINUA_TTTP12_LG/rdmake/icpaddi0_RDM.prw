...
IF cPaisLoc == "BRA"
   IF cExecute == "BOTAO"
      ...
      If Type("oDlgPrv") == "O"
         @ 004,055 BUTTON STR0259 SIZE 35,11 ACTION (IF(DIGerou(),Processa({|| PRETXT()},STR0031),)) PIXEL // EXPORTA
      Else
         @ 1.5,040 BUTTON STR0259 SIZE 37,11 ACTION (IF(DIGerou(),Processa({|| PRETXT()},STR0031),)) PIXEL // EXPORTA
      EndIf
      nOpca:=0
   ENDIF
   RETURN .T.
...
DO CASE
    ...
    CASE cExecute == 'OUTROS_INDICES'//'INDICE_D'
         ...
         IndRegua("Work1",Work1FileF+TEOrdBagExt(),"WK_NFE+WK_SE_NFE+WKFORN")
    ...
    CASE cExecute == "BOTAO" 
         IF cPaisLoc # "BRA"
            ...
            ENDIF
            //MFR 12/03/2019 OSSME-2258
            @ 04,95 BUTTON STR0259 SIZE 37,11 ACTION Processa({|| PRETXT()},STR0031)// GERAR ARQUIVO
            nOpca:=0
         ENDIF
...
IF nTipoNF # 4 .AND. (!lGeraNF .OR. (!lGerouNFE .AND. lNFAutomatica))
   RETURN MsgYesNo(STR0445,STR0022)//"Nao ha No. de Nota Fiscal! Deseja Exportar?"###"Atencao"
ENDIF

RETURN .T.
*-------------------------------------------------------------------------------------------------------*
//MFR 12/03/2019 OSSME-2258
Static FUNCTION PRETXT()
RETURN DI154PRTXT()
*-------------------------------------------------------------------------------------------------------*
