...
If POSICIONE('SE4',1,xFilial('SE4') + M->CJ_CONDPAG ,'E4_XTXFIX') == '1'.Or. at(UPPER(Alltrim( retcodusr() )),UPPER(cUsers)) > 0 
...
IF !empty(("TMP1")->CK_PRODUTO)
	While !Eof()
		If (cVariavel == 'M->CJ_XVLFRET' .And.  !("TMP1")->CK_FLAG .And. !Val(TMP1->CK_XMOTIVO) > 1 ) .Or. (IsinCallStack('U_A415LIOK') .And. M->CJ_XTPFRET == '3') //Valor digitado na mão ou chamado na pos validação
			nValor := nValDigit
			TMP1->CK_XVLFRET := nValDigit * (TMP1->CK_QTDVEN / nTotQtd )
			U_ClcComis()
		Else
			If  !("TMP1")->CK_FLAG .And. !Val(TMP1->CK_XMOTIVO) > 1 .And. M->CJ_XTPFRET <> '2'
				SB1->(dbSetOrder(1))
...
EndIf
If M->CJ_XTPFRET <> '3' .And. M->CJ_XTPFRET <> '2'
	M->CJ_XVLFRETE := nValor
Else
	nValor	:= nValDigit
EndIf

If !isincallstack('U_APRBID')
	DbSelectArea("TMP1")
	TMP1->(DbGotop())  
	Do While TMP1->(!eof())
...
If cAlias == 'DA1'
	...
	SBZ->(DbSetOrder(1))
	SBZ->(DbSeek(xFilial('SBZ') + oMdlDA1:GetValue('DA1_CODPRO')))
	nMargem		:= SBZ->BZ_MARKUP//U_DXGETMKP(oMdlDA1:GetValue('DA1_CODPRO'),TMP1->CK_QTDVEN)
	If nMargem == 0
...
ElseIf cAlias == 'SCJ'
	...
	If DA1->(DbSeek(xFilial('DA1') + SUPERGETMV('ES_DAXTAB',.T.,'001',cFilAnt) + cCodProd))	.And. !empty(cCondPag)
		...
		If DA1->DA1_MOEDA == 2
			cMoeda	:= 'U$ '
			nMoedaV	:= 2
			If '00' $ ALLTRIM(POSICIONE('SE4',1,xFilial('SE4') + cCondPag , 'E4_COND')) 
				TMP1->CK_XFIXA := 'S'
		...
		nMargem		:= U_DXGETMKP(cCodProd,TMP1->CK_QTDVEN)
		If nMargem == 0
	...
	nValMerc  	:= TMP1->CK_PRCVEN * TMP1->CK_QTDVEN
...
ElseIf cAlias == 'SBZ'
	...
	Else
		...
		nMargem		:= 0//U_DXGETMKP(cCodProd,TMP1->CK_QTDVEN)
		If nMargem == 0
...
If lContinua
		...
		If SB1->B1_ORIGEM $ '3|5'
			...
			If !(cAliasQry)->(EOF())
				If (cAliasQry)->CFD_ORIGEM == '3'
					...
					EndIf	nVlrCof := ((nValMerc * nAliqCOF)/100)
	...
	nRet := nPrcVen * ((1/(100 - (nAliqICM + nAliqPIS + nAliqCOF + nMargem ))) * 100) // calculo com base 18
	nRet := (nRet * (100 - nAliqICM - nAliqPIS - nAliqCOF)) / (100 - nAliqICM - nAliqPIS - nAliqCOF - nAliqDif)  //tratamento para base reduzida
	...
	If cAlias != 'SBZ' .and. cAlias != 'SCJ' // rodolfo - precificação - tratamento removido pois nao faz sentido utilizar na precificação
		Do Case
			Case nAliqICM == 0
				nRet := DA1->DA1_XPRCM0
			Case nAliqICM == 4
	...
	If cAlias == 'UPD'// Atualizo a margem do pedido
		If Round(nRet,2) <> Round(SCK->CK_PRUNIT,2)		
			nMgLiq		:= ((SCK->CK_PRCVEN * nQtdVen) - (nPrcVen * nQtdVen)) - nVlrPis - nVlrCof - nVlrIcm - nDifal -  nCustFin
...
If cAlias == 'SCJ'
	...
	TMP1->CK_XPRCUNI := nRet //gravo o valor num campo de referencia

	nRet += U_AddFrete()
	//If TMP1->CK_PRCVEN == 0
	TMP1->CK_XPRCVEN := nRet
	TMP1->CK_PRCVEN := TMP1->CK_XPRCVEN * (1 - TMP1->CK_DESCONT)
	TMP1->CK_VALOR := Round(TMP1->CK_QTDVEN*TMP1->CK_PRCVEN,6)   
	//EndIf
	TMP1->CK_CLASFIS := CodSitTri()
	If !ISINCALLSTACK( "U_UpdTotal" ) //tratamento para nao chamar recorrentemente
...
Local nPDsr		:= 0
Local nPCustFin		:= SupergetMV('ES_CUSTFIN',.T.,2.5)
Local cOpzeror := SUPERGETMV('ES_OPZEROR',.T.,'') 
SA3->(DbSetOrder(1))
...
nMargem := nPrcVen - nCustd - RetImp(nPrcVen) -(((nPrcVen + Round((TMP1->CK_XVLIPI * nPrcVen) / 100,2)) *  RetCfin(M->CJ_CONDPAG,nPCustFin) )/100 ) //valor do preço
...
If SA3->A3_XTIPO == '1' .And. SA3->A3_TIPO == 'E' //PF  e externo
	TMP1->CK_COMIS1 := SA3->A3_COMIS
	nRet := SA3->A3_COMIS
EndIF
...
If M->CJ_STATUS == 'A' .Or. Empty(M->CJ_STATUS)//INCLUI .OR. ALTERA
	TMP1->CK_XMGBRUT	:= nMargem
	If M->CJ_XTPOPER $ cOpzeror
		TMP1->CK_XMGBRUT	:= 0
		nRet				:= 0
	EndIf	
EndIf
...
EndIf
  ...
  //DEFINE MSDIALOG oDlg TITLE "Margem de Rentabilidade" FROM 000, 000  TO 800, 1500 COLORS 0, 16777215 PIXEL
   DEFINE MSDIALOG oDlg TITLE "Margem de Rentabilidade" FROM aSize[7], 000  TO 8000, 1500 COLORS 0, 16777215 PIXEL
	...
	@ 170, nCol1  SAY oSay10 PROMPT "(+) Icms de Frete" SIZE 092, 024 OF oDlg COLORS 0, 16777215 PIXEL
	@ 180, nCol1  SAY oSay10 PROMPT "(-) Custo de Palete" SIZE 092, 024 OF oDlg COLORS 0, 16777215 PIXEL
	@ 190, nCol1  SAY oSay10 PROMPT "(=) Margem"  FONT oFont16n SIZE 150, 50 OF oDlg COLORS 0, 16777215 PIXEL
	@ 200, nCol1  SAY oSay10 PROMPT "(-) Comissão de vendas sem DSR"  SIZE 150, 50 OF oDlg COLORS 0, 16777215 PIXEL
	@ 210, nCol1  SAY oSay10 PROMPT "(-) DSR sobre comissão de vendas"  SIZE 150, 50 OF oDlg COLORS 0, 16777215 PIXEL
	@ 220, nCol1  SAY oSay10 PROMPT "(=) Comissão com DSR"  FONT oFont16n SIZE 150, 50 OF oDlg COLORS 0, 16777215 PIXEL
	@ 240, nCol1  SAY oSay10 PROMPT "(=) Resultado Financeiro" FONT oFont16n SIZE 150, 50 OF oDlg COLORS 0, 16777215 PIXEL
	...
	*/
	...
	@ 170, nCol2 SAY oSay10 PROMPT Alltrim(Transform( aCol2[18,1], "@E 999,999,999,999.99" ))  SIZE 092, 024 OF oDlg COLORS 0, 16777215 PIXEL //ICMS DE FRETE
	@ 180, nCol2 SAY oSay10 PROMPT Alltrim(Transform( aCol2[12,1], "@E 999,999,999,999.99" ))  SIZE 092, 024 OF oDlg COLORS 0, 16777215 PIXEL //CUSTO DE Palete
	@ 190, nCol2 SAY oSay10 PROMPT Alltrim(Transform( aCol2[13,1], "@E 999,999,999,999.99" ))  FONT oFont16n SIZE 092, 024 OF oDlg COLORS 0, 16777215 PIXEL //Margem
	@ 200, nCol2 SAY oSay10 PROMPT Alltrim(Transform( aCol2[14,1], "@E 999,999,999,999.99" ))  SIZE 092, 024 OF oDlg COLORS 0, 16777215 PIXEL //Comissão s DSR
	@ 210, nCol2 SAY oSay10 PROMPT Alltrim(Transform( aCol2[15,1], "@E 999,999,999,999.99" ))  SIZE 092, 024 OF oDlg COLORS 0, 16777215 PIXEL //DSR
	@ 220, nCol2 SAY oSay10 PROMPT Alltrim(Transform( aCol2[16,1], "@E 999,999,999,999.99" ))  FONT oFont16n SIZE 092, 024 OF oDlg COLORS 0, 16777215 PIXEL //Comissão c DSR
	@ 240, nCol2 SAY oSay10 PROMPT Alltrim(Transform( aCol2[17,1], "@E 999,999,999,999.99" ))  FONT oFont16n  SIZE 092, 024 OF oDlg COLORS 0, 16777215 PIXEL //RESULTADO FINANCEIRO
	...
	@ 170, nCol2 + 60 SAY oSay10 PROMPT Alltrim(Transform( aCol2[18,2], "@E 999,999,999,999.99" ))  SIZE 092, 024 OF oDlg COLORS 0, 16777215 PIXEL	//ICMS DE FRETE
	@ 180, nCol2 + 60 SAY oSay10 PROMPT Alltrim(Transform( aCol2[12,2], "@E 999,999,999,999.99" ))  SIZE 092, 024 OF oDlg COLORS 0, 16777215 PIXEL	//CUSTO DE Palete
	@ 190, nCol2 + 60 SAY oSay10 PROMPT Alltrim(Transform( aCol2[13,2], "@E 999,999,999,999.99" ))  FONT oFont16n SIZE 092, 024 OF oDlg COLORS 0, 16777215 PIXEL //Margem
	@ 200, nCol2 + 60 SAY oSay10 PROMPT Alltrim(Transform( aCol2[14,2], "@E 999,999,999,999.99" ))  SIZE 092, 024 OF oDlg COLORS 0, 16777215 PIXEL //Comissão s DSR
	@ 210, nCol2 + 60 SAY oSay10 PROMPT Alltrim(Transform( aCol2[15,2], "@E 999,999,999,999.99" ))  SIZE 092, 024 OF oDlg COLORS 0, 16777215 PIXEL //DSR
	@ 220, nCol2 + 60 SAY oSay10 PROMPT Alltrim(Transform( aCol2[16,2], "@E 999,999,999,999.99" ))  FONT oFont16n SIZE 092, 024 OF oDlg COLORS 0, 16777215 PIXEL //Comissão c DSR
	@ 240, nCol2 + 60 SAY oSay10 PROMPT Alltrim(Transform( aCol2[17,2], "@E 999,999,999,999.99" ))  FONT oFont16n  SIZE 092, 024 OF oDlg COLORS 0, 16777215 PIXEL //RESULTADO FINANCEIRO
	...
	@ 170, nCol3 SAY oSay10 PROMPT Alltrim(Transform( aCol3[18,1], "@E 999,999,999,999.99" ))  SIZE 092, 024 OF oDlg COLORS 0, 16777215 PIXEL
	@ 180, nCol3 SAY oSay10 PROMPT Alltrim(Transform( aCol3[12,1], "@E 999,999,999,999.99" ))  SIZE 092, 024 OF oDlg COLORS 0, 16777215 PIXEL
	@ 190, nCol3 SAY oSay10 PROMPT Alltrim(Transform( aCol3[13,1], "@E 999,999,999,999.99" ))  FONT oFont16n SIZE 092, 024 OF oDlg COLORS 0, 16777215 PIXEL //Margem
	@ 200, nCol3 SAY oSay10 PROMPT Alltrim(Transform( aCol3[14,1], "@E 999,999,999,999.99" ))  SIZE 092, 024 OF oDlg COLORS 0, 16777215 PIXEL //Comissão s DSR
	@ 210, nCol3 SAY oSay10 PROMPT Alltrim(Transform( aCol3[15,1], "@E 999,999,999,999.99" ))  SIZE 092, 024 OF oDlg COLORS 0, 16777215 PIXEL //DSR
	@ 220, nCol3 SAY oSay10 PROMPT Alltrim(Transform( aCol3[16,1], "@E 999,999,999,999.99" )) FONT oFont16n SIZE 092, 024 OF oDlg COLORS 0, 16777215 PIXEL //Comissão c DSR
	@ 240, nCol3 SAY oSay10 PROMPT Alltrim(Transform( aCol3[17,1], "@E 999,999,999,999.99" ))  FONT oFont16n  SIZE 092, 024 OF oDlg COLORS 0, 16777215 PIXEL //RESULTADO FINANCEIRO
	...
	@ 170, nCol3 + 60 SAY oSay10 PROMPT Alltrim(Transform( aCol3[18,2], "@E 999,999,999,999.99" ))  SIZE 092, 024 OF oDlg COLORS 0, 16777215 PIXEL
	@ 180, nCol3 + 60 SAY oSay10 PROMPT Alltrim(Transform( aCol3[12,2], "@E 999,999,999,999.99" ))  SIZE 092, 024 OF oDlg COLORS 0, 16777215 PIXEL
	@ 190, nCol3 + 60 SAY oSay10 PROMPT Alltrim(Transform( aCol3[13,2], "@E 999,999,999,999.99" ))  FONT oFont16n SIZE 092, 024 OF oDlg COLORS 0, 16777215 PIXEL //Margem
	@ 200, nCol3 + 60 SAY oSay10 PROMPT Alltrim(Transform( aCol3[14,2], "@E 999,999,999,999.99" ))  SIZE 092, 024 OF oDlg COLORS 0, 16777215 PIXEL //Comissão s DSR
	@ 210, nCol3 + 60 SAY oSay10 PROMPT Alltrim(Transform( aCol3[15,2], "@E 999,999,999,999.99" ))  SIZE 092, 024 OF oDlg COLORS 0, 16777215 PIXEL //DSR
	@ 220, nCol3 + 60 SAY oSay10 PROMPT Alltrim(Transform( aCol3[16,2], "@E 999,999,999,999.99" )) FONT oFont16n SIZE 092, 024 OF oDlg COLORS 0, 16777215 PIXEL //Comissão c DSR
	@ 240, nCol3 + 60 SAY oSay10 PROMPT Alltrim(Transform( aCol3[17,2], "@E 999,999,999,999.99" ))  FONT oFont16n  SIZE 092, 024 OF oDlg COLORS 0, 16777215 PIXEL //RESULTADO FINANCEIRO
	...
	@ 170, nCol4 SAY oSay10 PROMPT Alltrim(Transform( aCol4[18,1], "@E 999,999,999,999.99" ))  SIZE 092, 024 OF oDlg COLORS 0, 16777215 PIXEL
	@ 180, nCol4 SAY oSay10 PROMPT Alltrim(Transform( aCol4[12,1], "@E 999,999,999,999.99" ))  SIZE 092, 024 OF oDlg COLORS 0, 16777215 PIXEL
	@ 190, nCol4 SAY oSay10 PROMPT Alltrim(Transform( aCol4[13,1], "@E 999,999,999,999.99" ))  FONT oFont16n SIZE 092, 024 OF oDlg COLORS 0, 16777215 PIXEL //Margem
	@ 200, nCol4 SAY oSay10 PROMPT Alltrim(Transform( aCol4[14,1], "@E 999,999,999,999.99" ))  SIZE 092, 024 OF oDlg COLORS 0, 16777215 PIXEL //Comissão s DSR
	@ 210, nCol4 SAY oSay10 PROMPT Alltrim(Transform( aCol4[15,1], "@E 999,999,999,999.99" ))  SIZE 092, 024 OF oDlg COLORS 0, 16777215 PIXEL //DSR
	@ 220, nCol4 SAY oSay10 PROMPT Alltrim(Transform( aCol4[16,1], "@E 999,999,999,999.99" )) FONT oFont16n  SIZE 092, 024 OF oDlg COLORS 0, 16777215 PIXEL //Comissão c DSR
	@ 240, nCol4 SAY oSay10 PROMPT Alltrim(Transform( aCol4[17,1], "@E 999,999,999,999.99" ))  FONT oFont16n  SIZE 092, 024 OF oDlg COLORS 0, 16777215 PIXEL //RESULTADO FINANCEIRO
	...
	@ 170, nCol4 + 60 SAY oSay10 PROMPT Alltrim(Transform( aCol4[18,2], "@E 999,999,999,999.99" ))  SIZE 092, 024 OF oDlg COLORS 0, 16777215 PIXEL
	@ 180, nCol4 + 60 SAY oSay10 PROMPT Alltrim(Transform( aCol4[12,2], "@E 999,999,999,999.99" ))  SIZE 092, 024 OF oDlg COLORS 0, 16777215 PIXEL
	@ 190, nCol4 + 60 SAY oSay10 PROMPT Alltrim(Transform( aCol4[13,2], "@E 999,999,999,999.99" ))  FONT oFont16n SIZE 092, 024 OF oDlg COLORS 0, 16777215 PIXEL //Margem
	@ 200, nCol4 + 60 SAY oSay10 PROMPT Alltrim(Transform( aCol4[14,2], "@E 999,999,999,999.99" ))  SIZE 092, 024 OF oDlg COLORS 0, 16777215 PIXEL //Comissão s DSR
	@ 210, nCol4 + 60 SAY oSay10 PROMPT Alltrim(Transform( aCol4[15,2], "@E 999,999,999,999.99" ))  SIZE 092, 024 OF oDlg COLORS 0, 16777215 PIXEL //DSR
	@ 220, nCol4 + 60 SAY oSay10 PROMPT Alltrim(Transform( aCol4[16,2], "@E 999,999,999,999.99" )) FONT oFont16n SIZE 092, 024 OF oDlg COLORS 0, 16777215 PIXEL //Comissão c DSR
	@ 240, nCol4 + 60 SAY oSay10 PROMPT Alltrim(Transform( aCol4[17,2], "@E 999,999,999,999.99" ))  FONT oFont16n  SIZE 092, 024 OF oDlg COLORS 0, 16777215 PIXEL //RESULTADO FINANCEIRO		
...
Local nTMP1Bkp		:= 0
Local nRef			:= 0
Local cTab 			:= SUPERGETMV('ES_DAXTAB',.T.,'001',cFilAnt)
Local cOpzeror 		:= SUPERGETMV('ES_OPZEROR',.T.,'')
Local cTransp		:= ''
Local cNumTab		:= ''
Local cUfDest		:= ''
...
TMP1->(DbGoTop())
While !TMP1->(EOF())
	IF !TMP1->CK_FLAG .And. (!Val(TMP1->CK_XMOTIVO) > 1 .or. !lOrcTotal)
		nMoedaC		:=  1
		nMoedaV		:=  1
		cCodProd 	:= 	TMP1->CK_PRODUTO
		nQtdVen 	:=  TMP1->CK_QTDVEN
		...
		If cTipo $ '3' .Or. cTipo == '2' 
			if cTipo == '2'
				...
				If SZC->(DbSeek(xFilial('SZC') + M->CJ_NUM + '1'))
					...
					nTotIcFrt	+= SZC->ZC_ICMFRET
					nExcecoes	+= SZC->ZC_PALET
					nMgBruta 	+= SZC->ZC_RECEITA - SZC->ZC_CUSTD
					nTotMgLiq	+= nRecVen - nCustRep - nPis - nCofins - nIcms - nTotDifal - nTotFrete - nTotIcFrt - nExcecoes - nTotDespFin
					nDsr		+= SZC->ZC_DSR
					nItens		+= SZC->ZC_ITENS
					aItens		:= {}
					lSZL		:= .T.
			...
			DA1->(DbSetOrder(1))
			If DA1->(DbSeek(xFilial('DA1') + cTab  + cCodProd))	.And. !empty(cConPag)
				SBZ->(DbSetOrder(1)) //--B1_FILIAL+B1_COD
				SBZ->(DbSeek(FwxFilial('SBZ')+cCodProd))
...
While !TMP1->(EOF()) .And. !lSZL
	IF !TMP1->CK_FLAG .And. Len(aItens) > 0 .And. (!Val(TMP1->CK_XMOTIVO) > 1 .or. !lOrcTotal)
		nVlrPis  := 0
		nAliqPIS := 0
		nVlrCof  := 0
		nAliqCOF := 0
	...
	//	nIcmsP		+= nVlrIcmP
		...
		nMgLiq		:= (nNegociado - nPrcVen) - nVlrPis - nVlrCof - nVlrIcm - nDifal - nDespFin
...
nTotIcFrt := ((nTotFrete * nIcmFrete)/100)
...
aadd(aRet,{ROUND(nTotIcFrt ,2) 	, round((nTotIcFrt * 100) / nTotFrete ,2)})  //Icms Frete 18
MafisEnd()
TMP1->(DbGoTo(nLinha))
Return aRet
...
*/
  ...
  @ 050, nCol2 SAY oSay10 PROMPT Alltrim(Transform( aCol2[1,1], "@E 999,999,999,999.99" )) FONT oFont16n  SIZE 092, 024 OF oDlg COLORS 0, 16777215 PIXEL //Receita de vendas (com IPI)
	...
	@ 150, nCol2 + 60 SAY oSay10 PROMPT Alltrim(Transform( aCol2[10,2], "@E 999,999,999,999.99" ))  SIZE 092, 024 OF oDlg COLORS 0, 16777215 PIXEL	//DESPESAS FINANCEIRAS
	@ 160, nCol2 + 60 SAY oSay10 PROMPT Alltrim(Transform( aCol2[11,2], "@E 999,999,999,999.99" ))  SIZE 092, 024 OF oDlg COLORS 0, 16777215 PIXEL	//CUSTO DE FRETE
	@ 170, nCol2 + 60 SAY oSay10 PROMPT Alltrim(Transform( aCol2[18,2], "@E 999,999,999,999.99" ))  SIZE 092, 024 OF oDlg COLORS 0, 16777215 PIXEL	//CUSTO DE FRETE
	@ 180, nCol2 + 60 SAY oSay10 PROMPT Alltrim(Transform( aCol2[12,2], "@E 999,999,999,999.99" ))  SIZE 092, 024 OF oDlg COLORS 0, 16777215 PIXEL	//CUSTO DE Palete
	@ 190, nCol2 + 60 SAY oSay10 PROMPT Alltrim(Transform( aCol2[13,2], "@E 999,999,999,999.99" ))  FONT oFont16n SIZE 092, 024 OF oDlg COLORS 0, 16777215 PIXEL //Margem
	@ 200, nCol2 + 60 SAY oSay10 PROMPT Alltrim(Transform( aCol2[14,2], "@E 999,999,999,999.99" ))  SIZE 092, 024 OF oDlg COLORS 0, 16777215 PIXEL //Comissão s DSR
	@ 210, nCol2 + 60 SAY oSay10 PROMPT Alltrim(Transform( aCol2[15,2], "@E 999,999,999,999.99" ))  SIZE 092, 024 OF oDlg COLORS 0, 16777215 PIXEL //DSR
...
Local nPResult	:= 0
Local nVlResult	:= 0
Local cOpzerpe 	:= SUPERGETMV('ES_OPZERPE',.T.,'')
Private oFont16n:= TFont():New(,9,14,.T.,.T.,5,.T.,5,.T.,.F.)
Private oFont16	:= TFont():New(,9,14,.F.,.F.,5,.T.,5,.T.,.F.)
Private	nSeleciona	:= 0
Private nRecSC6	:= 0
...
DbSelectArea('SZD')
SZD->(Dbsetorder(1))
If SZD->(DbSeek(xFilial('SZD',SC5->C5_FILIAL) + Alltrim(Str(year(dDatabase))) + PadL(Alltrim(STR(Month(dDataBase))),2,'0')))
    nPerDsr		:= SZD->ZD_PDSR / 100    
EndIf

If Posicione('SA3',1,xFilial('SA3',SC5->C5_FILIAL) + SC5->C5_VEND1 ,'A3_XTIPO') == '2' 
	nPerDsr := 0
EndIf
...
If SC6->(dbseek(SC5->C5_FILIAL + SC5->C5_NUM))
	While SC5->C5_FILIAL + SC5->C5_NUM == SC6->(C6_FILIAL + C6_NUM)
			...
			If SZL->(DbSeek(SC5->C5_FILIAL + SC5->C5_XNUMCJ + SC6->C6_ITEM + SC6->C6_PRODUTO))
				lSZL := .T.
			...
			aadd(aItens,{SC6->C6_ITEM,;
								...
								Iif( SC5->C5_XTPOPER $ cOpzerpe ,'0%', Transform( nPResult, "@E 999,999,999.99" ) + ' %'),;
...
*/
@ 050, nCol2 SAY oSay10 PROMPT Alltrim(Transform( aCol2[1,1], "@E 999,999,999,999.99" ))FONT oFont16n  SIZE 092, 024 OF oDlg COLORS 0, 16777215 PIXEL //Receita de vendas (com IPI)
@ 060, nCol2 SAY oSay10 PROMPT Alltrim(Transform( aCol2[2,1], "@E 999,999,999,999.99" )) SIZE 092, 024  OF oDlg COLORS 0, 16777215 PIXEL // IPI
@ 080, nCol2 SAY oSay10 PROMPT Alltrim(Transform( aCol2[4,1], "@E 999,999,999,999.99" )) SIZE 150, 024 OF oDlg COLORS 0, 16777215 PIXEL //Custo de Reposição (Sem Impostos Recuperaveis)
@ 070, nCol2 SAY oSay10 PROMPT Alltrim(Transform( aCol2[3,1], "@E 999,999,999,999.99" )) FONT oFont16n SIZE 092, 024 OF oDlg COLORS 0, 16777215 PIXEL // Receita de vendas (sem IPI)
@ 090, nCol2 SAY oSay10 PROMPT Alltrim(Transform( aCol2[5,1], "@E 999,999,999,999.99" )) FONT oFont16n SIZE 092, 024 OF oDlg COLORS 0, 16777215 PIXEL //Geração Bruta de Caixa
@ 110, nCol2 SAY oSay10 PROMPT Alltrim(Transform( aCol2[6,1], "@E 999,999,999,999.99" )) SIZE 092, 024 OF oDlg COLORS 0, 16777215 PIXEL //Pis
...
@ 070, nCol2 + 50 SAY oSay10 PROMPT Alltrim(Transform( aCol2[3,2], "@E 999,999,999,999.99" ) ) FONT oFont16n SIZE 092, 024 OF oDlg COLORS 0, 16777215 PIXEL	//Receita de vendas
...
@ 170, nCol2 + 50 SAY oSay10 PROMPT Alltrim(Transform( aCol2[18,2], "@E 999,999,999,999.99" ))  SIZE 092, 024 OF oDlg COLORS 0, 16777215 PIXEL	//CUSTO DE FRETE
@ 180, nCol2 + 50 SAY oSay10 PROMPT Alltrim(Transform( aCol2[12,2], "@E 999,999,999,999.99" ))  SIZE 092, 024 OF oDlg COLORS 0, 16777215 PIXEL	//CUSTO DE PALETE	
@ 190, nCol2 + 50 SAY oSay10 PROMPT Alltrim(Transform( aCol2[13,2], "@E 999,999,999,999.99" ))  FONT oFont16n SIZE 092, 024 OF oDlg COLORS 0, 16777215 PIXEL //Margem
@ 200, nCol2 + 50 SAY oSay10 PROMPT Alltrim(Transform( aCol2[14,2], "@E 999,999,999,999.99" ))  SIZE 092, 024 OF oDlg COLORS 0, 16777215 PIXEL //Comissão s DSR
@ 210, nCol2 + 50 SAY oSay10 PROMPT Alltrim(Transform( aCol2[15,2], "@E 999,999,999,999.99" ))  SIZE 092, 024 OF oDlg COLORS 0, 16777215 PIXEL //DSR
@ 220, nCol2 + 50 SAY oSay10 PROMPT Alltrim(Transform( aCol2[16,2], "@E 999,999,999,999.99" )) 	FONT oFont16n SIZE 092, 024 OF oDlg COLORS 0, 16777215 PIXEL //Comissão c DSR
@ 240, nCol2 + 50 SAY oSay10 PROMPT Alltrim(Transform( aCol2[17,2], "@E 999,999,999,999.99" ))  FONT oFont16n  SIZE 092, 024 OF oDlg COLORS 0, 16777215 PIXEL //RESULTADO FINANCEIRO
...
While !Eof()
    If  !("TMP1")->CK_FLAG .And. !Val(TMP1->CK_XMOTIVO) > 1
        ...
        If SB1->(DbSeek(xFilial('SB1') + ("TMP1")->CK_PRODUTO )) .And. SB1->B1_PESBRU > 0
			...
			EndIf
			If M->CJ_XTPFRET == '7'
				TMP1->CK_XVLFRET := M->CJ_XVLFRET * (TMP1->CK_QTDVEN / nTotPeso )
			EndIf			
			If nLinha == TMP1->(Recno()) .And. M->CJ_XTPFRET <> '2'
				If M->CJ_XTPFRET $ '3|7' //CIF - DIGITADO NA MAO
...
If M->CJ_XTPFRET <> '3' .And. M->CJ_XTPFRET <> '2' .And.  M->CJ_XTPFRET <> '7'
...
If TMP1->CK_XPRCVEN > 0
	lRet := .F.
EndIf
If ISINCALLSTACK( "U_PVBID" )
	lRet := .F.
EndIf
Return lRet
...
Do While TMP1->(!eof())
	...
	if !TMP1->CK_FLAG .and. !empty(TMP1->CK_PRODUTO)   //ITEM do orcamento nao ta deletado
		...
		TMP1->CK_PRUNIT := U_DAXIMP('SCJ')
		TMP1->CK_PRCVEN := TMP1->CK_PRUNIT //ATUALIZO O PREÇO DE VENDA AQUI - PRECIFICAÇÂO
		U_ClcComis()
...
If IsInCallStack('MATA410') .And. SA3->(DbSeek(xFilial('SA3') + SC5->C5_VEND1))
	//Verifico se é PJ
	If SA3->A3_XTIPO == '2'  //PF  e externo
		nRet := SA3->A3_COMIS
	EndIF
EndIf
...
If SC6->(DbSeek(xFilial('SC6') + SC5->C5_NUM))
	While xFilial('SC6') + SC5->C5_NUM == SC6->(C6_FILIAL + C6_NUM)
		...
		//TRATAMENTO PARA PEDIDO DE AMOSTRA MOSTRAR TUDO ZERADO
		If POSICIONE('SCJ',1,xFilial('SCJ') + SC5->C5_XNUMCJ , 'CJ_XTPOPER') $ cOpzerpe 
			nRecVenIpi 	+= 0
			nIpi	   	+= 0
			nRecVen		+= 0
			nCustRep	+= 0
		...
		ElseIf cTipo $ '3' 
			DA1->(DbSetOrder(1))
			If DA1->(DbSeek(xFilial('DA1') + cCodTab + cCodProd))	.And. !empty(cCondPag)
				SBZ->(DbSetOrder(1)) //--B1_FILIAL+B1_COD
				SBZ->(DbSeek(FwxFilial('SBZ')+cCodProd))
...
EndIf
  ...
  //DEFINE MSDIALOG oDlg TITLE "Margem de Rentabilidade" FROM 000, 000  TO 800, 1500 COLORS 0, 16777215 PIXEL
	...
	*/
	...
	@ 150, nCol2 + 50 SAY oSay10 PROMPT Alltrim(Transform( aCol2[10,2], "@E 999,999,999,999.99" ))  SIZE 092, 024 OF oDlg COLORS 0, 16777215 PIXEL	//DESPESAS FINANCEIRAS
	@ 160, nCol2 + 50 SAY oSay10 PROMPT Alltrim(Transform( aCol2[11,2], "@E 999,999,999,999.99" ))  SIZE 092, 024 OF oDlg COLORS 0, 16777215 PIXEL	//CUSTO DE FRETE
	@ 170, nCol2 + 50 SAY oSay10 PROMPT Alltrim(Transform( aCol2[18,2], "@E 999,999,999,999.99" ))  SIZE 092, 024 OF oDlg COLORS 0, 16777215 PIXEL	//ICMS DE FRETE
	@ 180, nCol2 + 50 SAY oSay10 PROMPT Alltrim(Transform( aCol2[12,2], "@E 999,999,999,999.99" ))  SIZE 092, 024 OF oDlg COLORS 0, 16777215 PIXEL	//CUSTO DE Palete
	@ 190, nCol2 + 50 SAY oSay10 PROMPT Alltrim(Transform( aCol2[13,2], "@E 999,999,999,999.99" ))  FONT oFont16n SIZE 092, 024 OF oDlg COLORS 0, 16777215 PIXEL //Margem
	@ 200, nCol2 + 50 SAY oSay10 PROMPT Alltrim(Transform( aCol2[14,2], "@E 999,999,999,999.99" ))  SIZE 092, 024 OF oDlg COLORS 0, 16777215 PIXEL //Comissão s DSR
	@ 210, nCol2 + 50 SAY oSay10 PROMPT Alltrim(Transform( aCol2[15,2], "@E 999,999,999,999.99" ))  SIZE 092, 024 OF oDlg COLORS 0, 16777215 PIXEL //DSR
	...
	@ 170, nCol3 + 50 SAY oSay10 PROMPT Alltrim(Transform( aCol3[18,2], "@E 999,999,999,999.99" ))  SIZE 092, 024 OF oDlg COLORS 0, 16777215 PIXEL
	@ 180, nCol3 + 50 SAY oSay10 PROMPT Alltrim(Transform( aCol3[12,2], "@E 999,999,999,999.99" ))  SIZE 092, 024 OF oDlg COLORS 0, 16777215 PIXEL
	@ 190, nCol3 + 50 SAY oSay10 PROMPT Alltrim(Transform( aCol3[13,2], "@E 999,999,999,999.99" ))  FONT oFont16n SIZE 092, 024 OF oDlg COLORS 0, 16777215 PIXEL //Margem
	@ 200, nCol3 + 50 SAY oSay10 PROMPT Alltrim(Transform( aCol3[14,2], "@E 999,999,999,999.99" ))  SIZE 092, 024 OF oDlg COLORS 0, 16777215 PIXEL //Comissão s DSR
	@ 210, nCol3 + 50 SAY oSay10 PROMPT Alltrim(Transform( aCol3[15,2], "@E 999,999,999,999.99" ))  SIZE 092, 024 OF oDlg COLORS 0, 16777215 PIXEL //DSR
	@ 220, nCol3 + 50 SAY oSay10 PROMPT Alltrim(Transform( aCol3[16,2], "@E 999,999,999,999.99" )) FONT oFont16n SIZE 092, 024 OF oDlg COLORS 0, 16777215 PIXEL //Comissão c DSR
	@ 240, nCol3 + 50 SAY oSay10 PROMPT Alltrim(Transform( aCol3[17,2], "@E 999,999,999,999.99" ))  FONT oFont16n  SIZE 092, 024 OF oDlg COLORS 0, 16777215 PIXEL //RESULTADO FINANCEIRO
	...
	@ 170, nCol4 + 50 SAY oSay10 PROMPT Alltrim(Transform( aCol4[18,2], "@E 999,999,999,999.99" ))  SIZE 092, 024 OF oDlg COLORS 0, 16777215 PIXEL
	@ 180, nCol4 + 50 SAY oSay10 PROMPT Alltrim(Transform( aCol4[12,2], "@E 999,999,999,999.99" ))  SIZE 092, 024 OF oDlg COLORS 0, 16777215 PIXEL
	@ 190, nCol4 + 50 SAY oSay10 PROMPT Alltrim(Transform( aCol4[13,2], "@E 999,999,999,999.99" ))  FONT oFont16n SIZE 092, 024 OF oDlg COLORS 0, 16777215 PIXEL //Margem
	@ 200, nCol4 + 50 SAY oSay10 PROMPT Alltrim(Transform( aCol4[14,2], "@E 999,999,999,999.99" ))  SIZE 092, 024 OF oDlg COLORS 0, 16777215 PIXEL //Comissão s DSR
	@ 210, nCol4 + 50 SAY oSay10 PROMPT Alltrim(Transform( aCol4[15,2], "@E 999,999,999,999.99" ))  SIZE 092, 024 OF oDlg COLORS 0, 16777215 PIXEL //DSR
	@ 220, nCol4 + 50 SAY oSay10 PROMPT Alltrim(Transform( aCol4[16,2], "@E 999,999,999,999.99" )) FONT oFont16n SIZE 092, 024 OF oDlg COLORS 0, 16777215 PIXEL //Comissão c DSR
	@ 240, nCol4 + 50 SAY oSay10 PROMPT Alltrim(Transform( aCol4[17,2], "@E 999,999,999,999.99" ))  FONT oFont16n  SIZE 092, 024 OF oDlg COLORS 0, 16777215 PIXEL //RESULTADO FINANCEIRO		
...
While ( !Eof() )
	If ( !TMP1->CK_FLAG ) 
		TMP1->CK_PRUNIT := U_DAXIMP('SCJ',TMP1->CK_PRUNIT)
	EndIf
	dbSelectArea("TMP1")
	dbSkip()
EndDo
...
dbGotop()
While ( !Eof() )
	If ( !TMP1->CK_FLAG ) //.And. !Val(TMP1->CK_XMOTIVO) > 1 - removido a pedidos do wesley 29/04/2024 , para somar os perdidos no valor total
...
TMP1->(DbGoTo(nLinha))
If !IsInCallStack('U_A415LIOK') .And. !isincallstack('U_APRBID')
	GETDREFRESH()      
	oGetDad:Refresh()
EndIf
...
If aSX6[MV_FISAUCF] .And. FindFunction( "XFciGetOrigem" )
	...
	If !Empty(cCpoProd) .And. !Empty(cVarProd) .And. !Empty(dEmissao)
		...
		If ReadVar() $ cCpoProd .And. Type(cVarProd) <> "U"
			cProduto := &(cVarProd)
...
If M->CJ_XTPFRET == '4' .And. Empty(cTransp)
