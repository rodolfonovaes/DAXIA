...
WsRestful PosFinanceira Description "Lista de Posição Financeira" Format APPLICATION_JSON
    ...
    WSDATA searchIni           AS STRING   OPTIONAL
    WSDATA searchFim           AS STRING   OPTIONAL
    WSDATA searchType          AS STRING   OPTIONAL
...
/*/
//-------------------------------------------------------------------
WSMETHOD GET WSRECEIVE  searchIni,searchFim,searchType, page, pageSize, cgc WSSERVICE PosFinanceira
    lRet := PosFin( self )
Return( lRet )
...
Static Function PosFin( oSelf )
    ...
    Default oself:cgc       := ' '
    Default oself:searchIni := ' '
    Default oself:searchFim := 'ZZZZZ'
    Default oself:searchType := 'NF'
    ...
    Do Case
        Case Upper( oself:searchType ) == 'NF'
            cWhere += " AND SE1.E1_NUM BETWEEN '" + Alltrim(Upper( oself:searchIni )) + "' AND  '"  + Alltrim(Upper( oself:searchFim )) + "' "
        Case Upper( oself:searchType ) == 'EMISSAO'
            cWhere += " AND SE1.E1_EMISSAO BETWEEN '" + Alltrim(Upper( oself:searchIni )) + "' AND  '"  + Alltrim(Upper( oself:searchFim )) + "' "
        Case Upper( oself:searchType ) == 'VENCIMENTO'
            cWhere += " AND SE1.E1_VENCTO BETWEEN '" + Alltrim(Upper( oself:searchIni )) + "' AND  '"  + Alltrim(Upper( oself:searchFim )) + "' "            
        Case Upper( oself:searchType ) == 'PEDIDO'
            cWhere += " AND SE1.E1_PEDIDO BETWEEN '" + Alltrim(Upper( oself:searchIni )) + "' AND  '"  + Alltrim(Upper( oself:searchFim )) + "' "                        
        Case Upper( oself:searchType ) == 'NUMBOL'
            cWhere += " AND SE1.E1_NUMBOL BETWEEN '" + Alltrim(Upper( oself:searchIni )) + "' AND  '"  + Alltrim(Upper( oself:searchFim )) + "' "                                    
        Case Upper( oself:searchType ) == 'STATUS'
            cWhere += " AND SE1.E1_STATUS BETWEEN '" + Alltrim(Upper( oself:searchIni )) + "' AND  '"  + Alltrim(Upper( oself:searchFim )) + "' "     
    EndCase                                           
    ...
    While ( cAliasSE1 )->( ! Eof() )
        ...
        If nCount >= nStart
            ...
            aListFin[nAux]['parcela']       := ( cAliasSE1 )->E1_PARCELA
            ...
            aListFin[nAux]['status']   := ( cAliasSE1 )->E1_STATUS
