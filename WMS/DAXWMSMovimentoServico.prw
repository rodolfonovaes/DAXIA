...
//------------------------------------------------------------------------------
User Function WCLS0059()
Return Nil
//------------------------------------------------------------------------------
CLASS DAXWMSMovimentoServico FROM WMSDTCMovimentosServicoArmazem
...
METHOD New() CLASS DAXWMSMovimentoServico
	_Super:New()
	Self:aColetor := {}
	Self:aConfEnd := {}
Return
...
METHOD GetArrConf() CLASS DAXWMSMovimentoServico
Return Self:aConfEnd

METHOD IniArrCol() CLASS DAXWMSMovimentoServico
	Self:aColetor := {}
Return

METHOD IniArrConf() CLASS DAXWMSMovimentoServico
	Self:aConfEnd := {}
Return
...
/*/
//--------------------------------------------------
METHOD RecEnter() CLASS DAXWMSMovimentoServico
...
/*/
//--------------------------------------------------
METHOD RecExit() CLASS DAXWMSMovimentoServico
Local lRet       := .T.
...
/*/
//--------------------------------------------------
METHOD RunOutput() CLASS DAXWMSMovimentoServico
...
/*/
//--------------------------------------------------
METHOD RunInput() CLASS DAXWMSMovimentoServico
Local lRet       := .T.
Local lEmpenho   := .F.
Local lBloqueio  := .F.
Local lUsaCalc   := .T.
...
Return lRet
//-----------------------------------------------------------------------------
METHOD MkInOutOri() CLASS DAXWMSMovimentoServico
Local lRet      := .T.
Local lEmpPrev  := .F.
Local cQuery    := ""
Local cAliasD13 := ""
...
/*/
//--------------------------------------------------
METHOD UpdLibCrs() CLASS DAXWMSMovimentoServico
			...
			Do While lRet .And. (cAliasSC9)->(!Eof())
				...
				If oServico:LoadData() .And. oServico:HasOperac({'4'})
					...
					If lRet
						...
						RecLock("SC9",.F.)
						SC9->C9_BLEST := ""
						SC9->C9_XLOGEST   := UsrRetName( retcodusr() ) + ";" + DTOC(dDataBase) + ";" + Time()  + ";" + FUNNAME() + ";" + SC9->C9_BLEST
...
METHOD UpdUltMov() CLASS DAXWMSMovimentoServico
Local lRet         := .T.
Local cQuery       := ""
Local cAliasQry    := ""
Local aAreaAnt     := GetArea()
...
METHOD UpdSepMov() CLASS DAXWMSMovimentoServico
Local lRet        := .T.
Local cAliasQry   := ""
Local cQuery      := ""
Local aAreaAnt    := GetArea()
...
/*/
//--------------------------------------------------
METHOD UpdLibSer() CLASS DAXWMSMovimentoServico
Local lRet      := .T.
Local aAreaAnt  := GetArea()
Local cAliasD12 := ""
Local cQuery    := ""
...
/*/
//--------------------------------------------------
METHOD UpdEmpSDC() CLASS DAXWMSMovimentoServico
Local lRet        := .T.
...
/*/
//--------------------------------------------------
METHOD UpdLibVol() CLASS DAXWMSMovimentoServico
Local lRet      := .T.
Local oMntVol   := Nil
Local cAliasDCF := ""
Local cQuery    := ""
...
/*/
//--------------------------------------------------
METHOD MakeTrfSD3() CLASS DAXWMSMovimentoServico
Local lRet       := .T.
Local aAreaAnt   := GetArea()
Local cQuery     := ""
Local cAliasDH1  := Nil
...
/*/
//--------------------------------------------------
METHOD MakeUpdSD3() CLASS DAXWMSMovimentoServico
Local lRet       := .T.
Local lBaixaEst  := Self:GetBxEsto() == "1"
Local aAreaAnt   := GetArea()
Local aTamSX3    := TamSx3("DCR_QUANT")
...
/*/
//--------------------------------------------------
METHOD UpdMntVol(nQtde,cVolume) CLASS DAXWMSMovimentoServico
Local lRet        := .T.
Local cAliasQry   := ""
Local cQuery      := ""
Local aAreaAnt    := GetArea()
...
/*/
//--------------------------------------------------
METHOD HasUsrAtv(cRecHum,cFuncao,cUsuArma) CLASS DAXWMSMovimentoServico
Local aAreaD12  := D12->(GetArea())
Local lRet      := .F.
Local cQuery    := ""
Local cAliasD12 := ""
...
/*/
//--------------------------------------------------
METHOD OrdColetor(nOriDest) CLASS DAXWMSMovimentoServico
Local aAux := {}
Local i
...
METHOD UpdEndOri(cNewEnd,cNewUnit, cNewTip) CLASS DAXWMSMovimentoServico
...
/*/
//--------------------------------------------------
METHOD UpdEndDes(cNewEnd,cNewUnit, cNewTip) CLASS DAXWMSMovimentoServico
...
/*/
//--------------------------------------------------
METHOD UpdEndMov(nTipo,cNewEnd,cNewUnit,cNewTip) CLASS DAXWMSMovimentoServico
Local lRet      := .T.
Local aAreaD12  := D12->(GetArea())
Local cAliasD12 := ""
Local cQuery    := ""
...
/*/
//--------------------------------------------------
METHOD HitMovEst() CLASS DAXWMSMovimentoServico
Local lRet      := .T.
Local lEmpPrev  := Self:oMovServic:ChkSepara()
...
----------------------------------------------------------------------*/
METHOD ValCapReab() CLASS DAXWMSMovimentoServico
Local lRet       := .T.
Local nCapEnder  := 0
Local lPercOcup  := .F.
Local nSaldoD14  := 0
...
----------------------------------------------------------------------*/
METHOD ValMntExc() CLASS DAXWMSMovimentoServico
Local cQuery    := ""
Local cAliasQry := ""
Local lRet      := .T.
...
----------------------------------------------------------------------------------*/
METHOD HasPrdSep() CLASS DAXWMSMovimentoServico
Local lRet      := .F.
Local cQuery    := ""
...
---felipe.m 13/11/2015
---------------------------------------------------------*/
METHOD ChkSitMov() CLASS DAXWMSMovimentoServico
Local lRet := .T.
Local cQuery := ""
Local cAliasD12 := ""
...
/*/
METHOD ChkFinMov(aErroFin,nQuant) CLASS DAXWMSMovimentoServico
Local lRet       := .T.
...
METHOD HasMoreMov() CLASS DAXWMSMovimentoServico
Local lRet      := .F.
Local cQuery    := ""
Local cAliasD12 := ""
...
METHOD HasMoreEnd(nTipo,cEndereco) CLASS DAXWMSMovimentoServico
Local lRet      := .F.
Local cQuery    := ""
Local cAliasD12 := ""
...
METHOD UpdSldDis() CLASS DAXWMSMovimentoServico
Local lRet      := .T.
Local cQuery    := ""
Local cAliasTRP := ""
Local cAliasDCF := ""
...
METHOD AllMovPend() CLASS DAXWMSMovimentoServico
Local lRet      := .T.
Local cQuery    := ""
Local cAliasD12 := Nil 
	cQuery := "SELECT 1"
...
//FUNCAO AJUSTADA PARA NAO CONSIDERAR O SERVICO
/*-----------------------------------------------------------------------------
...
-----------------------------------------------------------------------------*/
Static Function XWmsAtuSC9(cCarga,cPedido,cItem,cProduto,cServico,cLoteCtl,cNumLote,cNumSerie,nQuant,nQuant2UM,cLocal,cEndereco,cIdDCF,nTipoRegra,xRegra,lWmsLibSC9,lGeraEmp)
Local aAreaAnt   := GetArea()
Local lRet       := .T.
Local cQuery     := ""
Local cAliasSC9  := ""
...
Default lGeraEmp   := .T.
	...
	cCarga   := PadR(cCarga,   Len(SC9->C9_CARGA))
	cPedido  := PadR(cPedido,  Len(SC9->C9_PEDIDO))
	cItem    := PadR(cItem,    Len(SC9->C9_ITEM))
	cProduto := PadR(cProduto, Len(SC9->C9_PRODUTO))
	cServico := PadR(cServico, Len(SC9->C9_SERVIC))

	cQuery := " SELECT C9_SEQUEN, SC9.R_E_C_N_O_ RECNOSC9 "
	...
	If cServico == '019'
		cQuery += " AND (C9_SERVIC  = '"+cServico+"' OR C9_SERVIC  = '025' ) "
	Else	
		cQuery += " AND C9_SERVIC  = '"+cServico+"'"
	EndIf
	If !lWmsNew
		If lLote .And. nTipoRegra == 1 .And. !Empty(xRegra)
			cQuery += " AND C9_LOTECTL     = '"+xRegra+"'"
		EndIf
	Else
	...
	While lRet .And. (cAliasSC9)->(!Eof()) .And. QtdComp(nQuant) > 0
		...
		If !lWmsLibSC9 .And. (!lLote .Or. (lLote .And. !Empty(SC9->C9_LOTECTL)))
			...
			If SDC->(DbSeek(xFilial("SDC")+cProduto+cLocal+"SC6"+cPedido+cItem+cSeqSC9+SC9->C9_LOTECTL+SC9->C9_NUMLOTE+cEndereco+SC9->C9_NUMSERI,.F.))
				nQtdLib -= SDC->DC_QUANT
			EndIf
		...
		Else
			If lWmsLibSC9 .Or.;
				...
				If lLote
					...
					If SB8->(DbSeek(xFilial('SB8')+cProduto+cLocal+cLoteCtl+Iif(lSLote,cNumLote,''), .F.))
	...
	If lRet .And. QtdComp(nQuant) > 0
		...
		cMensagem := STR0001+CRLF // "Não foi possível liberar toda a quantidade solicitada do pedido de venda."
		If WmsCarga(cCarga)
			cMensagem += Trim(RetTitle("C9_CARGA"))+": "+cCarga+" - "+Trim(RetTitle("DCF_ID"))+": "+cIdDCF+CRLF
		Else
			cMensagem += Trim(RetTitle("C9_PEDIDO"))+": "+cPedido+" - "+Trim(RetTitle("C9_ITEM"))+": "+cItem+" - "+Trim(RetTitle("DCF_ID"))+": "+cIdDCF+CRLF
		EndIf
		cMensagem += STR0002+AllTrim(Transf((nQuant+nQtdLibTot),PesqPictQt('C9_QTDLIB',14)))+CRLF //"Qtd Solicitada: "
...
Local cSeqSC9   := "01"
	...
	cQuery := "SELECT MAX(C9_SEQUEN) MAXSC9SEQ"
	cQuery +=  " FROM "+RetSqlName("SC9")+" SC9 "
	cQuery += " WHERE C9_FILIAL  = '"+xFilial("SC9")+"'"
	cQuery +=   " AND C9_PEDIDO  = '"+cPedido+"'"
	cQuery +=   " AND C9_ITEM    = '"+cItem+"'"
