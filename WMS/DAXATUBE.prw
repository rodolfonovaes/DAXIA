...
If File( cFileImp )
	...
	If !(nHandle == -1)
        ...
        nPFilial      := Ascan(aLine,'BE_FILIAL')
        nLOCAL        := Ascan(aLine,'BE_LOCAL')
        nLOCALIZ      := Ascan(aLine,'BE_LOCALIZ')
        nXID	      := Ascan(aLine,'BE_XID')
		...
		While !FT_FEof() 
			...
			Dbselectarea('SBE')
			SBE->(dbsetorder(1))									
			if  !SBE->(Dbseek(PADR(aLine[nPFilial],TAMSX3('BE_FILIAL')[1]) + PADR(aLine[nLOCAL],Tamsx3('BE_LOCAL')[1]) + PADR(aLine[nLOCALIZ],TAMSX3('BE_LOCALIZ')[1])))
				LjWriteLog(cLog, "Linha : " + Alltrim(STR(nLinha)) + ' Registro não encontrado na SB8! '+ aLine[nPFilial] + aLine[nLOCAL] + aLine[nLOCALIZ] )  
            Else
                Reclock('SBE',.F.)  
                SBE->BE_XID     := aLine[nXID]
                MsUnlock()
			EndIf
