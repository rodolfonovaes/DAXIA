...
Local cLocalCQ    := SuperGetMV("MV_CQ",.F.,"")
	...
	If Self:GetStServ() == "3"
		If WmsX312118("D13","D13_USACAL") .And. !(Self:oServico:HasOperac({'3','4'})) // Caso o serviço for de separação ou separação crossdocking permite estorno
			...
			cQuery +=   " AND D13_IDDCF = '"+Self:cIdDCF+"'"
			cQuery +=   " AND D13_DTESTO < '"+DToS(MVUlmes())+"'"
		...
		EndIf
		If Self:oServico:HasOperac({'8'}) .And. Self:GetOrigem() == "D0A" // Caso serviço tenha operação de transferencia
			lRet := Self:HaveMovD0A()
		Else
			...
			If lRet
				If Self:oServico:GetTipo() == "2"
					If Self:cOrigem == "SC9" .And. !Self:lCarteira
						...
						cQuery +=   " AND C9_IDDCF = '"+Self:GetIdDCF()+"'"
					...
					EndIf
					If lRet
						// Verificar por produto movimentado
						D00->(dbSetOrder(1))
						If D00->(dbSeek(xFilial("D00")+Self:GetCarga()+AllTrim(Self:GetDocto())))
							Self:cErro := WmsFmtMsg(STR0008,{{"[VAR01]",Self:GetDocto()},{"[VAR02]",Self:oProdLote:GetProduto()},{"[VAR03]",Str(Self:GetQuant())},{"[VAR04]",Self:GetOrigem()}}) // "Existe endereçamento para o documento: [VAR01] produto: [VAR02] quantidade: [VAR03] origem: [VAR04]."
							lRet := .F.
							// Libera o estorno do registro uma vez que tentado realizar a opção de estorno do faturamento por carteira
							If Self:lCarteira
								RecLock("D00",.F.)
								D00->D00_LIBEST := "1"
								D00->(MsUnlock())
					...
					If lRet
						cQuery := "SELECT D02.R_E_C_N_O_ RECNOD02,"
						cQuery +=       " D02.D02_STATUS,"
						cQuery +=       " D0H.D0H_CODEXP"
						cQuery +=  " FROM "+RetSqlName("D0H")+" D0H"
						cQuery += " INNER JOIN "+RetSqlName('D02')+" D02"
						cQuery +=    " ON D02.D02_FILIAL = '"+xFilial("D02")+"'"
						cQuery +=   " AND D02.D02_CODEXP = D0H.D0H_CODEXP"
						If !Empty(Self:GetCarga()) .And. WMSCarga(Self:GetCarga())
							cQuery += " AND D02.D02_CARGA = '"+Self:GetCarga()+"'"
						EndIf
						cQuery +=   " AND D02.D02_PEDIDO = '"+Self:GetDocto()+ "'"
						cQuery +=   " AND D02.D02_PRDORI = '"+Self:oProdLote:GetPrdOri()+"'"
						If !Self:oProdLote:oProduto:oProdComp:IsDad()
							cQuery +=   " AND D02.D02_CODPRO = '"+Self:oProdLote:GetProduto()+ "'"
						EndIf
						If !Empty(Self:oProdLote:GetLoteCtl())
							cQuery +=   " AND D02.D02_LOTE   = '"+Self:oProdLote:GetLoteCtl()+"'"
						EndIf
						If !Empty(Self:oProdLote:GetNumLote())
							cQuery +=   " AND D02.D02_SUBLOT = '"+Self:oProdLote:GetNumLote()+"'"
						EndIf
						cQuery +=   " AND D02.D_E_L_E_T_ = ' '"
						cQuery += " WHERE D0H.D0H_FILIAL = '"+xFilial("D0H")+"'"
						cQuery +=   " AND D0H.D0H_IDDCF  = '"+Self:GetIdDCF()+"'"
					...
					If lRet
						cQuery := "SELECT DCT.R_E_C_N_O_ RECNODCT,"
						cQuery +=       " DCT.DCT_STATUS,"
						cQuery +=       " D0I.D0I_CODMNT"
						cQuery +=  " FROM "+RetSqlName("D0I")+" D0I"
						cQuery += " INNER JOIN "+RetSqlName('DCT')+" DCT"
						cQuery +=    " ON DCT.DCT_FILIAL = '"+xFilial("DCT")+"'"
						cQuery +=   " AND DCT.DCT_CODMNT = D0I.D0I_CODMNT"
						If !Empty(Self:GetCarga()) .And. WmsCarga(Self:GetCarga())
							cQuery += " AND DCT.DCT_CARGA = '"+Self:GetCarga()+"'"
						EndIf
						cQuery +=   " AND DCT.DCT_PEDIDO = '"+Self:GetDocto()+"'"
						cQuery +=   " AND DCT.DCT_PRDORI = '"+Self:oProdLote:GetPrdOri()+"'"
						If !Self:oProdLote:oProduto:oProdComp:IsDad()
							cQuery += " AND DCT.DCT_CODPRO = '"+Self:oProdLote:GetProduto()+"'"
						EndIf
						If !Empty(Self:oProdLote:GetLoteCtl())
							cQuery +=   " AND DCT.DCT_LOTE   = '"+Self:oProdLote:GetLoteCtl()+"'"
						EndIf
						If !Empty(Self:oProdLote:GetNumLote())
							cQuery +=   " AND DCT.DCT_SUBLOT = '"+Self:oProdLote:GetNumLote()+"'"
						EndIf
						cQuery +=   " AND DCT.D_E_L_E_T_ = ' '"
						cQuery += " WHERE D0I.D0I_FILIAL = '"+xFilial("D0I")+"'"
						cQuery +=   " AND D0I.D0I_IDDCF  = '"+Self:GetIdDCF()+"'"
					...
					If lRet
						...
						cQuery +=       " D0J.D0J_CODDIS"
						cQuery +=  " FROM "+RetSqlName("D0J")+" D0J"
						cQuery += " INNER JOIN "+RetSqlName('D0E')+" D0E"
						cQuery +=    " ON D0E.D0E_FILIAL = '"+xFilial("D0E")+"'"
						cQuery +=   " AND D0E.D0E_CODDIS = D0J.D0J_CODDIS"
						If !Empty(Self:GetCarga()) .And. WmsCarga(Self:GetCarga())
							cQuery += " AND D0E.D0E_CARGA = '" + Self:GetCarga() + "'"
						EndIf
						cQuery +=   " AND D0E.D0E_PEDIDO = '"+Self:GetDocto()+"'"
						cQuery +=   " AND D0E.D0E_PRDORI = '"+Self:oProdLote:GetPrdOri()+"'"
						If !Self:oProdLote:oProduto:oProdComp:IsDad()
							cQuery +=   " AND D0E.D0E_PRODUT = '"+Self:oProdLote:GetProduto()+"'"
						EndIf
						If !Empty(Self:oProdLote:GetLoteCtl())
							cQuery +=   " AND D0E.D0E_LOTECT = '"+Self:oProdLote:GetLoteCtl()+"'"
						EndIf
						If !Empty(Self:oProdLote:GetNumLote())
							cQuery +=   " AND D0E.D0E_NUMLOT = '"+Self:oProdLote:GetNumLote()+"'"
						EndIf
						cQuery +=   " AND D0E.D_E_L_E_T_ = ' '"
						cQuery += " WHERE D0J.D0J_FILIAL = '"+xFilial("D0J")+"'"
						cQuery +=   " AND D0J.D0J_IDDCF  = '"+Self:GetIdDCF()+"'"
					...
					If Self:cOrigem == "SD4"
						...
						cQuery +=                      " AND DCR.DCR_IDDCF  = '"+Self:cIdDCF+"'"
						cQuery +=                      " AND DCR.DCR_SEQUEN = '"+Self:cSequen+"'"
			...
			If lRet
				...
				cQuery +=                   " AND D12.D12_FILIAL = '"+xFilial("D12")+"'"
				cQuery +=                   " AND ((SBE.BE_LOCAL = D12.D12_LOCORI AND SBE.BE_LOCALIZ = D12.D12_ENDORI) "
				cQuery +=                     " OR (SBE.BE_LOCAL = D12.D12_LOCDES AND SBE.BE_LOCALIZ = D12.D12_ENDDES))"
				cQuery +=                   " AND SBE.BE_STATUS = '6'" // Bloqueio de inventário
	...
	If Self:LockDCF()
		If Self:GetStServ() == '3'
			...
			If lRet .And. Self:oServico:HasOperac({'3','4'}) // Caso serviço tenha operação de separação, separação crossdocking
				...
				EndIf
			EndIf
			// Valida se é serviço de transferencia de montagem ou desmontagem
			If lRet .And. Self:cOrigem == 'D0A' .And.  Self:oServico:HasOperac({'8'}) // Caso serviço tenha operação de transferencia
				If WmsArmUnit(Self:oOrdEndDes:GetArmazem())
					oDmdUnit := WMSDTCDemandaUnitizacao():New()
					oDmdUnit:SetDocto(Self:cDocumento)
					lRet := oDmdUnit:UndoMntDes()
				Else
					lRet := Self:UndoMntDes()
				EndIf
			EndIf
			If lRet
...
Local oConExpItem := Nil
	...
	cQuery +=   " AND D12.D12_STATUS <> '0'"
	cQuery +=   " AND D12.D_E_L_E_T_ = ' '"
	cQuery += " GROUP BY D12.D12_PRODUT, D12.D12_LOTECT, D12.D12_NUMLOT, D12.D12_NUMSER, DCR.DCR_IDDCF, D12.D12_STATUS"
	cQuery += " ORDER BY D12.D12_PRODUT, D12.D12_LOTECT, D12.D12_NUMLOT, D12.D12_NUMSER, DCR.DCR_IDDCF"
	cQuery := ChangeQuery(cQuery)
	cAliasD12 := GetNextAlias()
	DbUseArea(.T.,"TOPCONN",TcGenQry(,,cQuery),cAliasD12,.F.,.T.)
...
Local aArrProd    := Self:oProdLote:GetArrProd()
	...
	cQuery := "SELECT D12.D12_LOCORI,"
	cQuery +=       " D12.D12_PRDORI,"
	cQuery +=       " D12.D12_LOTECT,"
	cQuery +=       " D12.D12_NUMLOT,"
	cQuery +=       " (SUM(DCR.DCR_QUANT)/"+cValToChar(aArrProd[1,2])+") DCR_QUANT"
	cQuery +=  " FROM "+RetSqlName("DCR")+" DCR, "+RetSqlName("D12")+" D12"
	cQuery += " WHERE DCR.DCR_FILIAL = '"+xFilial("DCR")+"'"
	cQuery +=   " AND DCR.DCR_IDDCF  = '"+Self:cIdDCF+"'"
	cQuery +=   " AND DCR.DCR_SEQUEN = '"+Self:cSequen+"'"
	...
	cQuery +=   " AND D12.D12_IDMOV  = DCR.DCR_IDMOV"
	cQuery +=   " AND D12.D12_IDOPER = DCR.DCR_IDOPER"
	cQuery +=   " AND D12.D12_PRODUT = '"+aArrProd[1,1]+"'"
	cQuery +=   " AND D12.D12_PRDORI = '"+aArrProd[1,3]+"'"
	cQuery +=   " AND D12.D12_ATUEST = '1'"
	cQuery +=   " AND D12.D12_STATUS <> '0'"
	cQuery +=   " AND D12.D_E_L_E_T_ = ' '"
	cQuery += " GROUP BY D12.D12_LOCORI, D12.D12_PRDORI, D12.D12_LOTECT, D12.D12_NUMLOT"
	cQuery += " ORDER BY D12.D12_LOCORI, D12.D12_PRDORI, D12.D12_LOTECT, D12.D12_NUMLOT"
	cQuery := ChangeQuery(cQuery)
	cAliasD12 := GetNextAlias()
	DbUseArea(.T.,"TOPCONN",TcGenQry(,,cQuery),cAliasD12,.F.,.T.)
	TcSetField(cAliasD12,'DCR_QUANT','N',aTamSX3[1],aTamSX3[2])
...
Local lWmsBlqe  := SuperGetMV("MV_WMSBLQE",.F.,.F.)
	If Self:IsMovUnit()
		...
		cQuery +=        " D0Q.D0Q_QUANT,"
		cQuery +=        " D0Q.D0Q_CODPRO,"
		cQuery +=        " D0Q.D0Q_LOTECT,"
		cQuery +=        " D0Q.D0Q_NUMLOT"
		cQuery +=   " FROM "+RetSqlName('D0Q')+" D0Q"
		cQuery +=  " INNER JOIN "+RetSqlName('D0U')+" D0U"
		cQuery +=     " ON D0U.D0U_FILIAL = '"+xFilial('D0U')+"'"
		cQuery +=    " AND D0U.D0U_IDDCF  = D0Q.D0Q_ID"
		cQuery +=    " AND D0U.D0U_ORIGEM = 'D0Q'"
		cQuery +=    " AND D0U.D_E_L_E_T_ = ' '"
		cQuery +=  " WHERE D0Q.D0Q_FILIAL = '"+xFilial('D0Q')+"'"
		cQuery +=    " AND D0Q.D0Q_ID IN ( SELECT DISTINCT D0S.D0S_IDD0Q"
		...
		cQuery +=                        " AND DCF.DCF_UNITIZ = D0S.D0S_IDUNIT"
...
Local nQtdSldEnd := 0
	...
	If lRet .And. Self:oMovimento:IsUpdEst()
		...
		lBloqueio  := (Self:oMovimento:oOrdServ:GetOrigem() == 'D0A' .And. Self:oMovimento:oMovServic:ChkTransf())
		If Self:oMovimento:IsMovUnit()
			...
			cQuery +=  " FROM "+RetSqlName('D0S')+" D0S"
			cQuery += " WHERE D0S.D0S_FILIAL = '"+xFilial('D0S')+"'"
			cQuery +=   " AND D0S.D0S_IDUNIT = '"+Self:oMovimento:GetIdUnit()+"'"
			cQuery +=   " AND D0S.D_E_L_E_T_ = ' '"
			cQuery := ChangeQuery(cQuery)
			cAliasQry := GetNextAlias()
			DbUseArea(.T.,'TOPCONN',TcGenQry(,,cQuery),cAliasQry,.F.,.T.)
	...
	If lRet .And. Self:oMovimento:IsUpdEst()
		...
		If Self:oMovimento:IsMovUnit()
			...
			Else
				...
				cQuery +=   " AND D14.D14_IDUNIT = '"+Self:oMovimento:GetUniDes()+"'"
				cQuery +=   " AND D14.D14_LOCAL  = '"+Self:oMovimento:oMovEndDes:GetArmazem()+"'"
				cQuery +=   " AND D14.D14_ENDER  = '"+Self:oMovimento:oMovEndDes:GetEnder()+"'"
				cQuery +=   " AND D14.D14_QTDEPR > 0"
...
Local oEtiqUnit  := Nil
	...
	If lRet
		...
		If !oMovAux:ChkEndDes(Self:oMovimento:oOrdServ:GetOrigem() == 'D0A')
			If Self:oMovimento:oMovServic:ChkRecebi() .Or. Self:oMovimento:oOrdServ:GetOrigem() == 'D0A' .Or. (Self:oMovimento:oOrdServ:GetOrigem() $ 'SC9|SD4' .And. !Empty(Self:oMovimento:oOrdServ:oOrdEndOri:GetEnder()))
				Self:cErro := oMovAux:GetErro()
				lRet := .F.
			Else
...
Local oOrdSerAux:= Nil
	...
	If !(Self:oOrdEndOri:GetArmazem() == Self:oOrdEndDes:GetArmazem())
		cQuery := " SELECT DH1.R_E_C_N_O_ RECNODH1"
		cQuery +=   " FROM "+RetSqlName("DH1")+" DH1"
		cQuery +=  " WHERE DH1.DH1_FILIAL = '"+xFilial("DH1")+"'"
		cQuery +=    " AND DH1.DH1_IDDCF = '"+Self:GetIdDCF()+"'"
	...
	If (cAliasQry)->(!Eof())
		...
		Do While lRet .And. (cAliasQry)->(!Eof())
			...
			cQuery := " SELECT DISTINCT D12.D12_BXESTO"
			cQuery +=   " FROM "+RetSqlName("DCR")+" DCR, "+RetSqlName("D12")+" D12"
			cQuery +=  " WHERE DCR.DCR_FILIAL = '"+xFilial("DCR")+"'"
			cQuery +=    " AND DCR.DCR_IDDCF  = '"+Self:cIdDCF+"'"
			cQuery +=    " AND DCR.DCR_SEQUEN = '"+Self:cSequen+"'"
			...
			If (cAliasD12)->(!Eof())
				...
				If (cAliasD12)->D12_BXESTO == '1'
					...
					cQuery +=   " AND SD3.D3_DOC = '"+Self:cDocumento+"'"
					cQuery +=   " AND SD3.D3_OP = '"+SD4->D4_OP+"'"
					cQuery +=   " AND SD3.D3_TRT = '"+SD4->D4_TRT+"'"
					cQuery +=   " AND SD3.D3_IDDCF = '"+SD4->D4_IDDCF+"'"
...
Local oEstEnder  := Nil
	...
	cQuery +=   " AND D12.D12_PRDORI = '"+Self:oProdLote:GetProduto()+"'"
	cQuery +=   " AND D12.D12_SEQUEN = DCR.DCR_SEQUEN"
	cQuery +=   " AND D12.D12_IDDCF  = DCR.DCR_IDORI"
	cQuery +=   " AND D12.D12_IDMOV  = DCR.DCR_IDMOV"
	cQuery +=   " AND D12.D12_IDOPER = DCR.DCR_IDOPER"
	cQuery +=   " AND D12.D_E_L_E_T_ = ' '"
	cQuery +=   " AND D12.D12_ORDTAR = '"+Self:oServico:GetOrdem()+"'" // Assume a tarefa exatamante anterior
	cQuery +=   " AND D12.D12_ORDMOV IN ('3','4')"
	cQuery +=   " AND D12.D12_BXESTO = '1'" // Somente se baixa estoque
	cQuery +=   " AND NOT EXISTS (SELECT 1"
...
Local cAliasDCF := Nil
	...
	cQuery +=  " FROM "+RetSqlName("DCF")+" DCF"
	cQuery += " WHERE DCF.DCF_FILIAL = '"+xFilial("DCF")+"'"
	cQuery +=   " AND DCF.DCF_ID IN ("+cListIdDcf+")"
	cQuery +=   " AND EXISTS (SELECT 1"
	cQuery +=                 " FROM "+RetSqlName("DCF")+" DCF1"
	cQuery +=                " WHERE DCF1.DCF_FILIAL = '"+xFilial("DCF")+"'"
	cQuery +=                  " AND DCF1.DCF_SERVIC = DCF.DCF_SERVIC"
	cQuery +=                  " AND DCF1.DCF_DOCTO = DCF.DCF_DOCTO"
	cQuery +=                  " AND DCF1.DCF_STSERV = '2'"
