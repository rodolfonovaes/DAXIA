...
If File( cFileImp )
	...
	If !(nHandle == -1)
        ...
        nPFilial      := Ascan(aLine,'B8_FILIAL')
        nPProd        := Ascan(aLine,'B8_PRODUTO')
        nPLote        := Ascan(aLine,'B8_LOTECTL')
        nPLocal       := Ascan(aLine,'B8_LOCAL')
        nPLoteFor     := Ascan(aLine,'B8_LOTEFOR')
        nPClifor      := Ascan(aLine,'B8_CLIFOR')
        nPLoja        := Ascan(aLine,'B8_LOJA')
        nPXFabri      := Ascan(aLine,'B8_XCFABRI')
        nPXLFabri     := Ascan(aLine,'B8_XLFABRI')
        nPFabric      := Ascan(aLine,'B8_NFABRIC')
        nPXPais       := Ascan(aLine,'B8_XPAISOR')
        nPDFabric     := Ascan(aLine,'B8_DFABRIC')
		// Pula cabecalho
		FT_FSkip()
		...
		While !FT_FEof() 
			...
			Dbselectarea('SB8')
			SB8->(dbsetorder(3))									
			if  !SB8->(Dbseek(PADR(aLine[nPFilial],TAMSX3('B8_FILIAL')[1]) + PADR(aLine[nPProd],Tamsx3('B8_PRODUTO')[1]) + PADR(aLine[nPLocal],TAMSX3('B8_LOCAL')[1]) + PADR(aLine[nPLote],TAMSX3('B8_LOTECTL')[1])))
				LjWriteLog(cLog, "Linha : " + Alltrim(STR(nLinha)) + ' Registro não encontrado na SB8! '+ aLine[nPFilial] + aLine[nPProd] + aLine[nPLocal] + aLine[nPLote])  
            Else
                Reclock('SB8',.F.)  
                SB8->B8_LOTEFOR := aLine[nPLoteFor]
                SB8->B8_CLIFOR  := aLine[nPClifor]
                SB8->B8_LOJA    := aLine[nPLoja]
                SB8->B8_XCFABRI := aLine[nPXFabri]
                SB8->B8_XLFABRI := aLine[nPXLFabri]
                SB8->B8_NFABRIC := aLine[nPFabric]
                SB8->B8_XPAISOR := aLine[nPXPais]
                SB8->B8_DFABRIC := STOD(aLine[nPDFabric])
                MsUnlock()
			EndIf
