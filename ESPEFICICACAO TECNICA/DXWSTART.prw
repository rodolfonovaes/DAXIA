...
Integração da geração da revisao de especificação tecnica com o fluig
...
User Function DXWSTART(cFile,cFileName,cProduto,nTipo)
	...
	cUser :=  Alltrim(SuperGetMV("ES_USRFLUIG", .F.,"elton.nicolau",))
	cPass :=  Alltrim(SuperGetMV("ES_PSWFLUIG", .F.,"Daxia@fluig2021",))
    ...
    If Len(aClientes) > 0
        //Montando mensagem de Requisição
        ...
        cMsg += '         <companyId>' + '1' + '</companyId>'+CRLF
        ...
        Else
            Alert('Erro na integração com o Fluig ' + cError)
        EndIF
    Else
        //Alert('')
    EndIf
...
If nTipo == 1
    // pego o periodo
    cQuery := "SELECT DISTINCT QE6_DTINI  "
    cQuery += "  FROM " + RetSQLTab('QE6') 
    cQuery += "  WHERE  "
    cQuery += "  QE6_FILIAL= " + xFilial('QE6') + "  "
    cQuery += " AND  QE6_PRODUT = '" + cProduto +"' "
    cQuery += "  AND D_E_L_E_T_ = ' '"
    cQuery += "  ORDER BY QE6_DTINI DESC "
    ...
    IF !(cAliasQry)->(EOF()) 
        dDataIni := STOD((cAliasQry)->QE6_DTINI)

        (cAliasQry)->(dbSkip())
        IF  !(cAliasQry)->(EOF()) 
            dDataFim := STOD((cAliasQry)->QE6_DTINI)
        EndIF
    EndIf
...
Else
    // pego o periodo
    cQuery := "SELECT DISTINCT QP6_DTINI  "
    cQuery += "  FROM " + RetSQLTab('QP6') 
    cQuery += "  WHERE  "
    cQuery += "  QP6_FILIAL= " + xFilial('QP6') + "  "
    cQuery += " AND  QP6_PRODUT = '" + cProduto +"' "
    cQuery += "  AND D_E_L_E_T_ = ' '"
    cQuery += "  ORDER BY QP6_DTINI DESC "
    ...
    IF !(cAliasQry)->(EOF()) 
        dDataIni := STOD((cAliasQry)->QP6_DTINI)

        (cAliasQry)->(dbSkip())
        IF  !(cAliasQry)->(EOF()) 
            dDataFim := STOD((cAliasQry)->QP6_DTINI)
        EndIF
    EndIf
...
//Pego os Clientes
cQuery := "SELECT DISTINCT SA1.A1_COD ,SA1.A1_LOJA , SA1.A1_NOME , SA1.A1_EMAIL  "
cQuery += "  FROM " + RetSQLTab('SD2') 
cQuery += "  INNER JOIN " + RetSQLTab('SA1') + " ON A1_COD = D2_CLIENTE AND A1_LOJA = D2_LOJA"
cQuery += "  WHERE  "
...
While !(cAliasQry)->(EOF()) 
    aadd(aRet,{(cAliasQry)->A1_COD + (cAliasQry)->A1_LOJA , LimpaSpec((cAliasQry)->A1_NOME) , IIF(EMPTY(GETMV('ES_MAILTST')),(cAliasQry)->A1_EMAIL, GETMV('ES_MAILTST'))})
    (cAliasQry)->(DbSkip())
EndDo
Return aRet 
