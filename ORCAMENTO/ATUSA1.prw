...
If File( cFileImp )
	...
	If !(nHandle == -1)
		...
		While !FT_FEof() 
			...
			Dbselectarea('SA1')
			SA1->(dbsetorder(1))									
			if  !SA1->(Dbseek(xFilial('SA1') +PADR(aLine[nPCod],TAMSX3('A1_COD')[1]) + PADR(aLine[nPLoja],Tamsx3('A1_LOJA')[1]) ))
				LjWriteLog(cLog, "Linha : " + Alltrim(STR(nLinha)) + ' Registro não encontrado na SA1! '+ aLine[nPCod] + aLine[nPLoja] )  
            Else
                Reclock('SA1',.F.)  
                SA1->A1_DTCAD := CTOD(aLine[4])
                MsUnlock()
			EndIf
