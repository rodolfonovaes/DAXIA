...
// CICERO CRUZ - INICIO - TRATAMENTO DA  REPETIÇÃO DA MENSAGEM
LOCAL lC5Msg 	:= .F.
...
If cTipo == "1"
	...
	For nCountNF := 1 To Len(aCupRefLoj)
		...
		aReboqu2	:= {}
		// CICERO - INICIO
		//lC5Msg      := .F. // PARA CADA NOVA  NOTA RE0NICIO A  MENSAGEM 
		cMensFis    := ""
		cMensCli    := ""
		// CICERO - FIM
...
/*******************************************************************************************************************************
DAXIA INICIO - MENSAGEM PARA O CLIENTE CUSTOMIZADA - CICERO CRUZ
********************************************************************************************************************************/	
					IF SA1->( ColumnPos( "A1_XMSG" ) ) > 0 .AND. !Empty( SA1->A1_XMSG ) 
						nLinhas     :=  LEN( ALLTRIM(SA1->A1_XMSG) ) / MAXMENLIN
						IF LEN(ALLTRIM(SA1->A1_XMSG) ) <= MAXMENLIN-1
							cMensCli	:= 	PADL( ALLTRIM(SA1->A1_XMSG), MAXMENLIN," ") 
						ELSE
							cMensCli	:= 	PADL( ALLTRIM(SA1->A1_XMSG), nLinhas * MAXMENLIN," ") 
						ENDIF
					ENDIF

					If !Empty(SA1->A1_MENSAGE) .and. SA1->(Formula(A1_MENSAGE)) <> Nil .and. !EMPTY(SA1->(Formula(A1_MENSAGE)))
						nLinhas     :=  LEN( ALLTRIM(SA1->(Formula( A1_MENSAGE )) ) ) / MAXMENLINV
						IF  LEN( ALLTRIM( SA1->( Formula( A1_MENSAGE ) ) ) )  <= MAXMENLIN-1
							//cCRZMSG :=  "CRZ001 - " + PADL( ALLTRIM( SA1->(Formula(A1_MENSAGE)) ), MAXMENLIN," ") 
							cCRZMSG :=  " " + PADL( ALLTRIM( SA1->(Formula(A1_MENSAGE)) ), MAXMENLIN," ") 
						ELSE
							//cCRZMSG :=  "CRZ002 - " + PADL( ALLTRIM( SA1->(Formula(A1_MENSAGE)) ), nLinhas * MAXMENLIN," ") 
							cCRZMSG :=  " " + PADL( ALLTRIM( SA1->(Formula(A1_MENSAGE)) ), nLinhas * MAXMENLIN," ") 
						ENDIF
						If cMVNFEMSA1=="C"
							cMensCli	+=	IIF( ( SA1->( ColumnPos( "A1_XMSG" ) ) > 0 .AND. !Empty( SA1->A1_XMSG ) ) , CRLF + "", "" ) + ALLTRIM( cCRZMSG )  + CRLF
							//cMensCli	+=	IIF( ( SA1->( ColumnPos( "A1_XMSG" ) ) > 0 .AND. !Empty( SA1->A1_XMSG ) ) , CRLF + "", "" ) + ALLTRIM( SA1->(Formula(A1_MENSAGE)) )  + CRLF
						ElseIf cMVNFEMSA1=="F"
							cMensFis	+=	IIF( ( SA1->( ColumnPos( "A1_XMSG" ) ) > 0 .AND. !Empty( SA1->A1_XMSG ) ) , CRLF + "", "" ) + ALLTRIM( cCRZMSG )  + CRLF
							//cMensFis	+=	IIF( ( SA1->( ColumnPos( "A1_XMSG" ) ) > 0 .AND. !Empty( SA1->A1_XMSG ) ) , CRLF + "", "" ) + ALLTRIM( SA1->(Formula(A1_MENSAGE)) )  + CRLF
						EndIf
					EndIf
...
/*******************************************************************************************************************************
DAXIA FINAL - MENSAGEM PARA O CLIENTE CUSTOMIZADA - CICERO CRUZ
********************************************************************************************************************************/	
			...
			/*	If SD2->(FieldPos("D2_DESCZFC"))<>0 .AND. SD2->(FieldPos("D2_DESCZFP"))<>0
				...
				dbSetOrder(3)
				#IFDEF TOP
					//RODOLFO - Tratamento na query para aglutinar os itens por produto e lote
					lQuery  := .T.
					...
					//Verifica se existe Template DCL
	      			IF cVerAmb >= "4.00" .And. (ExistTemplate("PROCMSG")) //Tratativa para Grupo de Repasse de Combustiveis
						BeginSql Alias cAliasSD2
								...
								D2_SEGURO,D2_PEDIDO,D2_ITEMPV,D2_DESPESA,D2_VALISS,D2_PRUNIT,
								D2_CLASFIS,D2_CODISS,D2_PREEMB,D2_LOTECTL,D2_NUMLOTE,D2_ICMSRET,D2_VALPS3,
								...
								(SELECT SUM(SD2A.D2_QUANT) FROM SD2010 SD2A 
										...
										SD2A.D2_SERIE   = %Exp:SF2->F2_SERIE% AND
										SD2A.D2_DOC     = %Exp:SF2->F2_DOC% AND
										SD2A.D2_CLIENTE = %Exp:SF2->F2_CLIENTE% AND
										SD2A.D2_LOJA    = %Exp:SF2->F2_LOJA% AND
										SD2A.%NotDel%
										AND SD2A.D2_COD = SD2.D2_COD
										AND SD2A.D2_LOTECTL = SD2.D2_LOTECTL
									) AS D2_QUANT,		
								(SELECT SUM(SD2B.D2_TOTAL) FROM SD2010 SD2B 
										WHERE
										SD2B.D2_FILIAL  = %xFilial:SD2% AND
										SD2B.D2_SERIE   = %Exp:SF2->F2_SERIE% AND
										SD2B.D2_DOC     = %Exp:SF2->F2_DOC% AND
										SD2B.D2_CLIENTE = %Exp:SF2->F2_CLIENTE% AND
										SD2B.D2_LOJA    = %Exp:SF2->F2_LOJA% AND
										SD2B.%NotDel%
										AND SD2B.D2_COD = SD2.D2_COD
										AND SD2B.D2_LOTECTL = SD2.D2_LOTECTL
									) AS D2_TOTAL,			
								(SELECT SUM(SD2C.D2_VALBRUT) FROM SD2010 SD2C 
										WHERE
										SD2C.D2_FILIAL  = %xFilial:SD2% AND
										SD2C.D2_SERIE   = %Exp:SF2->F2_SERIE% AND
										SD2C.D2_DOC     = %Exp:SF2->F2_DOC% AND
										SD2C.D2_CLIENTE = %Exp:SF2->F2_CLIENTE% AND
										SD2C.D2_LOJA    = %Exp:SF2->F2_LOJA% AND
										SD2C.%NotDel%
										AND SD2C.D2_COD = SD2.D2_COD
										AND SD2C.D2_LOTECTL = SD2.D2_LOTECTL
									) AS D2_VALBRUT,
								(SELECT TOP 1 (SD2D.D2_ITEM) FROM SD2010 SD2D
										WHERE
										SD2D.D2_FILIAL  = %xFilial:SD2% AND
										SD2D.D2_SERIE   = %Exp:SF2->F2_SERIE% AND
										SD2D.D2_DOC     = %Exp:SF2->F2_DOC% AND
										SD2D.D2_CLIENTE = %Exp:SF2->F2_CLIENTE% AND
										SD2D.D2_LOJA    = %Exp:SF2->F2_LOJA% AND
										SD2D.%NotDel%
										AND SD2D.D2_COD = SD2.D2_COD
										AND SD2D.D2_LOTECTL = SD2.D2_LOTECTL
										ORDER BY SD2D.D2_ITEM
									) AS D2_ITEM,
								(SELECT SUM(SD2E.D2_DESCZFP) FROM SD2010 SD2E
										WHERE
										SD2E.D2_FILIAL  = %xFilial:SD2% AND
										SD2E.D2_SERIE   = %Exp:SF2->F2_SERIE% AND
										SD2E.D2_DOC     = %Exp:SF2->F2_DOC% AND
										SD2E.D2_CLIENTE = %Exp:SF2->F2_CLIENTE% AND
										SD2E.D2_LOJA    = %Exp:SF2->F2_LOJA% AND
										SD2E.%NotDel%
										AND SD2E.D2_COD = SD2.D2_COD
										AND SD2E.D2_LOTECTL = SD2.D2_LOTECTL
									) AS D2_DESCZFP,	
								(SELECT SUM(SD2F.D2_DESCZFR) FROM SD2010 SD2F
										WHERE
										SD2F.D2_FILIAL  = %xFilial:SD2% AND
										SD2F.D2_SERIE   = %Exp:SF2->F2_SERIE% AND
										SD2F.D2_DOC     = %Exp:SF2->F2_DOC% AND
										SD2F.D2_CLIENTE = %Exp:SF2->F2_CLIENTE% AND
										SD2F.D2_LOJA    = %Exp:SF2->F2_LOJA% AND
										SD2F.%NotDel% AND
										SD2F.D2_COD 	= SD2.D2_COD AND
										SD2F.D2_LOTECTL = SD2.D2_LOTECTL
									) AS D2_DESCZFR,	
								(SELECT SUM(SD2G.D2_VALFRE) FROM SD2010 SD2G
										WHERE
										SD2G.D2_FILIAL  = %xFilial:SD2% AND
										SD2G.D2_SERIE   = %Exp:SF2->F2_SERIE% AND
										SD2G.D2_DOC     = %Exp:SF2->F2_DOC% AND
										SD2G.D2_CLIENTE = %Exp:SF2->F2_CLIENTE% AND
										SD2G.D2_LOJA    = %Exp:SF2->F2_LOJA% AND
										SD2G.%NotDel% AND
										SD2G.D2_COD 	= SD2.D2_COD AND
										SD2G.D2_LOTECTL = SD2.D2_LOTECTL
									) AS D2_VALFRE,		
								(SELECT SUM(SD2H.D2_DESCON) FROM SD2010 SD2H
										WHERE
										SD2H.D2_FILIAL  = %xFilial:SD2% AND
										SD2H.D2_SERIE   = %Exp:SF2->F2_SERIE% AND
										SD2H.D2_DOC     = %Exp:SF2->F2_DOC% AND
										SD2H.D2_CLIENTE = %Exp:SF2->F2_CLIENTE% AND
										SD2H.D2_LOJA    = %Exp:SF2->F2_LOJA% AND
										SD2H.%NotDel% AND
										SD2H.D2_COD 	= SD2.D2_COD AND
										SD2H.D2_LOTECTL = SD2.D2_LOTECTL
									) AS D2_DESCON	,
								(SELECT SUM(SD2I.D2_VALIPI) FROM SD2010 SD2I
										WHERE
										SD2I.D2_FILIAL  = %xFilial:SD2% AND
										SD2I.D2_SERIE   = %Exp:SF2->F2_SERIE% AND
										SD2I.D2_DOC     = %Exp:SF2->F2_DOC% AND
										SD2I.D2_CLIENTE = %Exp:SF2->F2_CLIENTE% AND
										SD2I.D2_LOJA    = %Exp:SF2->F2_LOJA% AND
										SD2I.%NotDel% AND
								...
								(SELECT SUM(SD2N.D2_BASIMP5) FROM SD2010 SD2N
									...
									(SELECT TOP 1 (SD2O.D2_PRCVEN)
									FROM SD2010 SD2O
									WHERE SD2O.D2_FILIAL 	= %xFilial:SD2% AND
										SD2O.D2_SERIE 	= %Exp:SF2->F2_SERIE% AND
										SD2O.D2_DOC 	= %Exp:SF2->F2_DOC% AND
										SD2O.D2_CLIENTE	= %Exp:SF2->F2_CLIENTE% AND
										SD2O.D2_LOJA 	= %Exp:SF2->F2_LOJA% AND
										SD2O.%NotDel% AND
										SD2O.D2_COD = SD2.D2_COD AND
										SD2O.D2_LOTECTL = SD2.D2_LOTECTL
									ORDER BY SD2O.D2_ITEM) AS D2_PRCVEN	 ,
								(SELECT SUM(SD2P.D2_DESCZFC) FROM SD2010 SD2P
										WHERE
										SD2P.D2_FILIAL  = %xFilial:SD2% AND
										SD2P.D2_SERIE   = %Exp:SF2->F2_SERIE% AND
										SD2P.D2_DOC     = %Exp:SF2->F2_DOC% AND
										...
										SD2P.%NotDel% AND
										SD2P.D2_COD 	= SD2.D2_COD AND
										SD2P.D2_LOTECTL = SD2.D2_LOTECTL
									) AS D2_DESCZFC,
									(SELECT TOP 1 (SD2Q.D2_IDENTB6)
									FROM SD2010 SD2Q
									WHERE SD2Q.D2_FILIAL 	= %xFilial:SD2% AND
										SD2Q.D2_SERIE 	= %Exp:SF2->F2_SERIE% AND
										SD2Q.D2_DOC 	= %Exp:SF2->F2_DOC% AND
										SD2Q.D2_CLIENTE	= %Exp:SF2->F2_CLIENTE% AND
										SD2Q.D2_LOJA 	= %Exp:SF2->F2_LOJA% AND
										SD2Q.%NotDel% AND
					...
					Else
						BeginSql Alias cAliasSD2
							...
							D2_SEGURO,D2_PEDIDO,D2_ITEMPV,D2_DESPESA,D2_VALISS,D2_PRUNIT,
							D2_CLASFIS,D2_CODISS,D2_PREEMB,D2_LOTECTL,D2_NUMLOTE,D2_ICMSRET,D2_VALPS3,
							D2_ORIGLAN,D2_VALCF3,D2_VALACRS,D2_PICM,D2_PDV ,
								(SELECT SUM(SD2A.D2_QUANT) FROM SD2010 SD2A 
										WHERE
										SD2A.D2_FILIAL  = %xFilial:SD2% AND
										SD2A.D2_SERIE   = %Exp:SF2->F2_SERIE% AND
		...
		//						If MsSeek(xFilial("SD2")+(cAliasSD2)->D2_NFORI+(cAliasSD2)->D2_SERIORI+(cAliasSD2)->D2_CLIENTE+(cAliasSD2)->D2_LOJA+(cAliasSD2)->D2_COD+(cAliasSD2)->D2_ITEMORI)
						...
						If !Empty(SC5->C5_MENPAD)
							//cCRZ003 := "CRZ003 - " + AllTrim(FORMULA(SC5->C5_MENPAD)) + " "
							cCRZ003 := " " + AllTrim(FORMULA(SC5->C5_MENPAD)) + " "
						EndIf
						If !Empty( cNumNfCup )
						...
						If SF4->(ColumnPos("F4_FORINFC") ) > 0  .And. !Empty(SF4->F4_FORINFC) .And. Formula(SF4->F4_FORINFC) <> NIL .And. ( ( cMVNFEMSF4=="C" .And. !AllTrim(Formula(SF4->F4_FORINFC)) $ cMensCli ) .Or. (cMVNFEMSF4=="F" .And. !AllTrim(Formula(SF4->F4_FORINFC))$cMensFis) )
							If cMVNFEMSF4=="C"
								If Len(cMensCli) > 0 .And. SubStr(cMensCli, Len(cMensCli), 1) <> " "
									cMensCli += " "
								EndIf
								//cMensCli	+=	"CRZ004 - " + SF4->(Formula(F4_FORINFC))
								cMensCli	+=	" " + SF4->(Formula(F4_FORINFC))
							ElseIf cMVNFEMSF4=="F"
								If Len(cMensFis) > 0 .And. SubStr(cMensFis, Len(cMensFis), 1) <> " "
									cMensFis += " "
								EndIf
								//cMensFis	+=	"CRZ005 - " + SF4->(Formula(F4_FORINFC))
						...
						ElseIf !Empty(SF4->F4_FORMULA) .And. Formula(SF4->F4_FORMULA) <> NIL .And. ( ( cMVNFEMSF4=="C" .And. !AllTrim(Formula(SF4->F4_FORMULA)) $ cMensCli ) .Or. (cMVNFEMSF4=="F" .And. !AllTrim(Formula(SF4->F4_FORMULA))$cMensFis) )
							If cMVNFEMSF4=="C"
								...
								******************************************************************************
								*/
								//cMensCli	+=	"CRZ006 - " + ALLTRIM( SF4->(Formula(F4_FORMULA)) )  + CRLF
								cMensCli	+=	" " + ALLTRIM( SF4->(Formula(F4_FORMULA)) )  + CRLF
							...
							ElseIf cMVNFEMSF4=="F"
								...
								******************************************************************************
								*/
								//cMensFis	+= "CRZ007 - " + ALLTRIM(	SF4->(Formula(F4_FORMULA)) ) + CRLF
								cMensFis	+= " " + ALLTRIM(	SF4->(Formula(F4_FORMULA)) ) + CRLF
					...
					//	cDescProd := IIF(Empty(SC6->C6_DESCRI),SB1->B1_DESC,SC6->C6_DESCRI) alterado conforme solicitação Rosalina - Rafael 14/01/2019
		...
		Else				
			...
			If SF1->F1_TIPO $ "DB" 
				...
				If !Empty(SA1->A1_MENSAGE) .and. SA1->(Formula(A1_MENSAGE)) <> Nil .and. !EMPTY(SA1->(Formula(A1_MENSAGE)))
					...
					*/
					If cMVNFEMSA1=="C"
						//cMensCli	:=	"CRZ008 - " + ALLTIM( SA1->(Formula(A1_MENSAGE)) ) + CRLF
						cMensCli	:=	" " + ALLTIM( SA1->(Formula(A1_MENSAGE)) ) + CRLF
					ElseIf cMVNFEMSA1=="F"
						//cMensFis	:=	"CRZ009 - " + ALLTIM( SA1->(Formula(A1_MENSAGE)) ) + CRLF
						cMensFis	:=	" " + ALLTIM( SA1->(Formula(A1_MENSAGE)) ) + CRLF
					EndIf
				...
				If SF1->(FieldPos("F1_MENPAD"))>0
					If !Empty(SF1->F1_MENPAD) .And. !AllTrim(FORMULA(SF1->F1_MENPAD)) $ cMensFis
						If Len(cMensFis) > 0 .And. SubStr(cMensFis, Len(cMensFis), 1) <> " "
							cMensFis += " "
						EndIf
						//cMensFis += "CRZ010 - " + AllTrim(FORMULA(SF1->F1_MENPAD))
					...
					If SF4->(ColumnPos("F4_FORINFC") ) > 0  .And. !Empty(SF4->F4_FORINFC) .And. Formula(SF4->F4_FORINFC) <> NIL .And. ( ( cMVNFEMSF4=="C" .And. !AllTrim(Formula(SF4->F4_FORINFC)) $ cMensCli ) .Or. (cMVNFEMSF4=="F" .And. !AllTrim(Formula(SF4->F4_FORINFC))$cMensFis) )
						If cMVNFEMSF4=="C"
							If Len(cMensCli) > 0 .And. SubStr(cMensCli, Len(cMensCli), 1) <> " "
								cMensCli += " "
							EndIf
							//cMensCli	+=	"CRZ011 - " + SF4->(Formula(F4_FORINFC))
							cMensCli	+=	" " + SF4->(Formula(F4_FORINFC))
						ElseIf cMVNFEMSF4=="F"
							If Len(cMensFis) > 0 .And. SubStr(cMensFis, Len(cMensFis), 1) <> " "
								cMensFis += " "
							EndIf
							//cMensFis	+=	"CRZ012 - " + SF4->(Formula(F4_FORINFC))
							cMensFis	+=	" " + SF4->(Formula(F4_FORINFC))
						EndIf					
					ElseIf !Empty(SF4->F4_FORMULA) .And. Formula(SF4->F4_FORMULA) <> NIL .And. ( ( cMVNFEMSF4=="C" .And. !AllTrim(Formula(SF4->F4_FORMULA)) $ cMensCli ) .Or. (cMVNFEMSF4=="F" .And. !AllTrim(Formula(SF4->F4_FORMULA))$cMensFis) )
						If cMVNFEMSF4=="C"
							If Len(cMensCli) > 0 .And. SubStr(cMensCli, Len(cMensCli), 1) <> " "
								cMensCli += " "
							EndIf
							//cMensCli	+=	"CRZ013 - " + SF4->(Formula(F4_FORMULA))
							cMensCli	+=	" " + SF4->(Formula(F4_FORMULA))
						ElseIf cMVNFEMSF4=="F"
							If Len(cMensFis) > 0 .And. SubStr(cMensFis, Len(cMensFis), 1) <> " "
								cMensFis += " "
							EndIf
							//cMensFis	+=	"CRZ014 - " + SF4->(Formula(F4_FORMULA))
							cMensFis	+=	" " + SF4->(Formula(F4_FORMULA))
						EndIf
					EndIf
				EndIf
