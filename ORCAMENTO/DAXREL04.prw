...
(
            ...
            (SELECT SUM(SE1.E1_SALDO) * AVG((SE1.E1_COMIS1)/100)
                FROM %TABLE:SE1% SE1            
                WHERE
                    SE1.E1_VEND1     = %Exp:aRet[1]% AND
                    SE1.E1_VENCREA  >= %Exp:DTOS(dDtIni)% AND
        ...
        While (cAliasQry)->( ! Eof() )
            ...
            oPrint:Say(nLinha, 1750     	, Transform((((cAliasQry)->CRECEB + (cAliasQry)->CARECEB) / (cAliasQry)->RECEBIDO) * 100  ,"@E 999,999.999")		        , oFont10 )
...
Static Function Cabecalho(oPrint, cAliasQry,aRet)
    ...
    oPrint:Say(0455, 1650, "% Medio comissão Sem DSR"	, oFont10)
    oPrint:Say(0455, 2050, "Comissão Recebida"	, oFont10)
    oPrint:Say(0455, 2300, "Comissão Atraso"	, oFont10)
    oPrint:Say(0455, 2600, "Comissão a Receber"	, oFont10)
