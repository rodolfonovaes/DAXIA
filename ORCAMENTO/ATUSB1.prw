...
If File( cFileImp )
	...
	If !(nHandle == -1)
		...
		While !FT_FEof() 
			...
			Dbselectarea('SB1')
			SB1->(dbsetorder(1))									
			if  !SB1->(Dbseek(xFilial('SA1') +PADR(aLine[nPCod],TAMSX3('B1_COD')[1])  ))
				LjWriteLog(cLog, "Linha : " + Alltrim(STR(nLinha)) + ' Registro não encontrado na SB1! '+ aLine[nPCod]  )  
            Else
                Reclock('SB1',.F.)  
