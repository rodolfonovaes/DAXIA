...
If File( cFileImp )
	...
	If !(nHandle == -1)
		...
		While !FT_FEof() 
            ...
            SC5->(DbSetOrder(1))
            SC6->(DbSetOrder(1))
            If SC5->(DbSeek(aLine[1] + aLine[2])) .and. aLine[1] == xFilial('SC5')
                If SC6->(DbSeek(SC5->C5_FILIAL + SC5->C5_NUM))
                    ...
                    While SC6->(C6_FILIAL + C6_NUM ) == SC5->(C5_FILIAL + C5_NUM) 
						...
						nRecTMP1 := SCK->(Recno())
						dDataEft :=  retdata()
						aDadosIt := U_PedRent('ITATUAL')//Rentab('ITATUAL')
						SC6->(DbGoTo(nRecSC6))
						SZL->(DbSetOrder(1))
						If SZL->(DbSeek(xFilial('SZL') + SC5->C5_XNUMCJ + cItem + SC6->C6_PRODUTO ))
							lReclock := .F.
						Else
							lReclock := .T.
						EndIf
						If Alltrim(aline[4]) == Alltrim(SC6->C6_PRODUTO)
							Reclock('SZL',lReclock)
							SZL->ZL_FILIAL  := xFilial('SZL')
							SZL->ZL_NUM     := SC5->C5_XNUMCJ 
							SZL->ZL_ITEM     := cItem 
							SZL->ZL_PRODUTO  := SC6->C6_PRODUTO 
...
While SCJ->(CJ_FILIAL + CJ_NUM ) == SCK->(CK_FILIAL+ CK_NUM)
	//IF !TMP1->CK_FLAG //.And. !Val(TMP1->CK_XMOTIVO) > 1
		...
		If cTipo $ '3' .Or. cTipo == '2' 
			...
			If DA1->(DbSeek(xFilial('DA1') + SUPERGETMV('ES_DAXTAB',.T.,'001',cFilAnt) + cCodProd))	.And. !empty(cConPag)
				...
				nPrcVen			:= xMoeda(DA1->DA1_XCUSTD,nMoedaC,1,dDataEft,TamSx3("C6_PRCVEN")[2],POSICIONE('SM2',1,dDataEft,'M2_MOEDA2')) * nQtdVen //transformo em real
				nPrcLista		:= nPrcVen
	...
	//		EndDo
			...
			If DA1->(DbSeek(xFilial('DA1') + SUPERGETMV('ES_DAXTAB',.T.,'001',cFilAnt) + cCodProd))	.And. !empty(cConPag)
				...
				EndIf
				//nPrcVen			:= TMP1->CK_PRCVEN	* nQtdVen//xMoeda(DA1->DA1_XCUSTD,nMoedaC,nMoedaV,dDataEft,TamSx3("C6_PRCVEN")[2],POSICIONE('SM2',1,dDataEft,'M2_MOEDA2'))
				nPrcLista			:= xMoeda(DA1->DA1_XCUSTD,val(SCK->CK_MOEDA),1,dDataEft,TamSx3("C6_PRCVEN")[2],POSICIONE('SM2',1,dDataEft,'M2_MOEDA2')) * nQtdVen //converto para real
				nPrcVen				:= nPrcLista
				nNegociado			:= SCK->CK_PRCVEN * SCK->CK_QTDVEN
				nVlrBase			:= xMoeda(SCK->CK_PRCVEN,VAL(SCK->CK_MOEDA),1,dDataEft,TamSx3("C6_PRCVEN")[2],POSICIONE('SM2',1,dDataEft,'M2_MOEDA2')) * SCK->CK_QTDVEN //transformo em real
