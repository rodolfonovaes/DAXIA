...
If File( cFileImp )
	...
	If !(nHandle == -1)
        ...
        nCodSeg := Ascan(aLine,'US_CODSEG')

        If nCodSeg == 0
            Alert('Não encontrou US_CODSEG!')
            Return 
        EndIf
        FT_FSkip()
		...
		While !FT_FEof() 
			...
			Dbselectarea('SUS')
			SUS->(dbsetorder(1))									
			if  !SUS->(Dbseek(xFilial('SUS') +PADR(aLine[nPCod],TAMSX3('US_COD')[1]) + PADR(aLine[nPLoja],Tamsx3('US_LOJA')[1]) ))
				LjWriteLog(cLog, "Linha : " + Alltrim(STR(nLinha)) + ' Registro não encontrado na SUS! '+ aLine[nPCod] + aLine[nPLoja] )  
            Else
                If Empty(SUS->US_CODSEG)
                    Reclock('SUS',.F.)  
                    SUS->US_CODSEG := aLine[nCodSeg]
                    MsUnlock()
                EndIf
			EndIf
