SELECT DISTINCT
	...
	(SELECT SUM(SD2A.D2_VALBRUT) FROM SD2010 SD2A
		 WHERE	SD2A.D2_DOC			= SD2.D2_DOC AND
				...
				SD2A.D2_LOTECTL     = SD2.D2_LOTECTL AND
	...
	(SELECT SUM(SD2B.D2_VALICM) FROM SD2010 SD2B
		 WHERE	SD2B.D2_DOC			= SD2.D2_DOC AND
				...
				SD2B.D2_LOTECTL     = SD2.D2_LOTECTL AND
	...
	(SELECT SUM(SD2C.D2_VALIPI) FROM SD2010 SD2C
		 WHERE	SD2C.D2_DOC			= SD2.D2_DOC AND
				...
				SD2C.D2_LOTECTL     = SD2.D2_LOTECTL AND
	...
	(SELECT SUM(SD2D.D2_VALFRE) FROM SD2010 SD2D
		 WHERE	SD2D.D2_DOC			= SD2.D2_DOC AND
				...
				SD2D.D2_LOTECTL     = SD2.D2_LOTECTL AND
	...
	(SELECT SUM(SD2E.D2_DESPESA) FROM SD2010 SD2E
		 WHERE	SD2E.D2_DOC			= SD2.D2_DOC AND
				...
				SD2E.D2_LOTECTL     = SD2.D2_LOTECTL AND
				SD2E.D_E_L_E_T_ = ' ') AS VL_DESPESA,
	 D2_TOTAL AS VL_FATURAMENTO_MERCADORIA, 
	/*(SELECT SUM(SD2F.D2_TOTAL) - SUM(SD2F.D2_VALDEV) FROM SD2010 SD2F
		 WHERE	SD2F.D2_DOC			= SD2.D2_DOC AND
				...
				SD2F.D2_LOTECTL     = SD2.D2_LOTECTL AND
	...
	(SELECT SUM(SD2G.D2_VALIMP6) FROM SD2010 SD2G
		 WHERE	SD2G.D2_DOC			= SD2.D2_DOC AND
				...
				SD2G.D2_LOTECTL     = SD2.D2_LOTECTL AND
				SD2G.D_E_L_E_T_ = ' ') AS VL_PIS_FATURAMENTO,
	/* D2_VALIMP5 AS VL_COFINS_FATURAMENTO, */
	(SELECT SUM(SD2G.D2_VALIMP5) FROM SD2010 SD2G
		 WHERE	SD2G.D2_DOC			= SD2.D2_DOC AND
				SD2G.D2_COD			= SD2.D2_COD AND
				SD2G.D2_CLIENTE		= SD2.D2_CLIENTE AND
				SD2G.D2_LOJA		= SD2.D2_LOJA AND
				SD2G.D2_LOTECTL     = SD2.D2_LOTECTL AND
				SD2G.D_E_L_E_T_ = ' ') AS VL_COFINS_FATURAMENTO,
	D2_QUANT AS QTD_FATURADA_ITEM,
	/*(SELECT SUM(SD2H.D2_QUANT) - SUM(SD2H.D2_QTDEDEV) FROM SD2010 SD2H
		 WHERE	SD2H.D2_DOC			= SD2.D2_DOC AND
				...
				SD2H.D2_LOTECTL     = SD2.D2_LOTECTL AND
	...
	(SELECT SUM(SD2I.D2_VALISS) FROM SD2010 SD2I
		 WHERE	SD2I.D2_DOC			= SD2.D2_DOC AND
				...
				SD2I.D2_LOTECTL     = SD2.D2_LOTECTL AND
	...
	(SELECT SUM(SD2J.D2_ICMSRET) FROM SD2010 SD2J
		 WHERE	SD2J.D2_DOC			= SD2.D2_DOC AND
				...
				SD2J.D2_LOTECTL     = SD2.D2_LOTECTL AND
	...
	(SELECT SUM(SD2L.D2_DESCON) FROM SD2010 SD2L
		 WHERE	SD2L.D2_DOC			= SD2.D2_DOC AND
				...
				SD2L.D2_LOTECTL     = SD2.D2_LOTECTL AND
	...
	DA1_MOEDA , 
	CASE
		WHEN C5_EMISSAO < '20200520' THEN 
			CASE 
			WHEN A3_XTIPO = '2' THEN SC6.C6_XMGBRUT - SC6.C6_COMIS1 /*/
							(SELECT COUNT(*) FROM SC6010 SC6
							 WHERE	SC6.C6_FILIAL = SD2.D2_FILIAL AND
									SC6.C6_NUM = SD2.D2_PEDIDO AND
									SC6.D_E_L_E_T_ = ' ' ) */
			ELSE SC6.C6_XMGBRUT - SC6.C6_COMIS1 -(SC6.C6_COMIS1 * (ZD_PDSR / 100))
			/*(((ZC_RECSIPI - ZC_CUSTD - ZC_PIS - ZC_COFINS - ZC_ICMS - ZC_ICMSP - ZC_FRETE - ZC_PALET - ZC_DESPFIN) * 100)/ SZC.ZC_RECSIPI - (ZC_PCOMIS * (ZD_PDSR / 100)) - ZC_PCOMIS) /
							(SELECT COUNT(*) FROM SC6010 SC6
							 WHERE	SC6.C6_FILIAL = SD2.D2_FILIAL AND
									SC6.C6_NUM = SD2.D2_PEDIDO AND
									SC6.D_E_L_E_T_ = ' ' ) */
	...
	CASE 
		WHEN C5_EMISSAO < '20200520' THEN 
				CASE 
					WHEN SC6.C6_XMOEDA = '2' AND SC6.C6_XFIXA = 'S' THEN
						CASE 
						WHEN A3_XTIPO = '2' THEN (SC6.C6_VALOR * SC6.C6_XTAXA) * ((C6_XMGBRUT - (SC6.C6_COMIS1 ))/100)
						ELSE (SC6.C6_VALOR * SC6.C6_XTAXA) * ((C6_XMGBRUT - (SC6.C6_COMIS1 +(SC6.C6_COMIS1 * (ZD_PDSR / 100))))/100)
						END 
					WHEN  SC6.C6_XMOEDA = '2' AND SC6.C6_XFIXA <> 'S' THEN
						CASE 
						WHEN A3_XTIPO = '2' THEN (SC6.C6_VALOR * SM2.M2_MOEDA2) * ((C6_XMGBRUT - (SC6.C6_COMIS1 ))/100)
						ELSE (SC6.C6_VALOR * SM2.M2_MOEDA2) * ((C6_XMGBRUT - (SC6.C6_COMIS1 +(SC6.C6_COMIS1 * (ZD_PDSR / 100))))/100)
						END 
					ELSE
						CASE 
						WHEN A3_XTIPO = '2' THEN (SC6.C6_VALOR ) * ((C6_XMGBRUT - (SC6.C6_COMIS1 ))/100)
						ELSE (SC6.C6_VALOR ) * ((C6_XMGBRUT - (SC6.C6_COMIS1 +(SC6.C6_COMIS1 * (ZD_PDSR / 100))))/100)
						END 
					END
		ELSE
			CASE 
...
LEFT JOIN SZL010 SZL
	...
	AND (SZL.ZL_LOTECTL = SD2.D2_LOTECTL OR SZL.ZL_LOTECTL = ' ')
	AND SZL.D_E_L_E_T_= ' ' 
...
WHERE D2_TIPO NOT IN ('B', 'D' ) 
	...
	AND D2_EMISSAO BETWEEN <<START_DATE>> AND <<FINAL_DATE>>
	AND F4_DUPLIC = 'S'
	AND F2_DUPL<>''
